/*! ableplayer V5.0.0-RC1 - with DOMPurify included. Console logs disabled, but not minified, for demos. */

(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
  typeof define === 'function' && define.amd ? define(factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.AblePlayer = factory());
  ...
  function addBuildplayerFunctions(AblePlayer) {
  	...
  	AblePlayer.prototype.cuePlaylistItem = function(sourceIndex) {
  		...
  		if (sourceSpans.length) {
  			sourceSpans.forEach(function(spanEl) {
  				...
  				if (thisObj.hasAttr(spanEl, "data-src")) {
  					const sanitizedSrc = purify.sanitize(spanEl.getAttribute("data-src"));
  			...
  			trackSpans.forEach(function(spanEl) {
  				if (thisObj.hasAttr(spanEl, "data-src") && thisObj.hasAttr(spanEl, "data-kind") && thisObj.hasAttr(spanEl, "data-srclang")) {
  					...
  					if (validate.isProtocolSafe(sanitizedSrc)) {
  						...
  						optionalAttributes.forEach((attr) => {
  							if (thisObj.hasAttr(spanEl, attr)) {
  								newTrack.setAttribute(attr, purify.sanitize(spanEl.getAttribute(attr)));
  							}
  						});
  		...
  		itemTitle = purify.sanitize( newItem.textContent );
  ...
  function addControlFunctions(AblePlayer) {
  	...
  	AblePlayer.prototype.setPlaybackRate = function (rate) {
  		...
  		this.speed.textContent = this.translate( 'speed', 'Speed' ) + ': ' + rate.toFixed(2).toString() + 'x';
  	};
  	...
  	AblePlayer.prototype.fadeControls = function(direction) {
  		...
  		if (direction == 'out') {
  			...
  			this.playerDiv.classList.add( 'fade-out' );
  			this.playerDiv.classList.remove( 'fade-in' );
  		} else if (direction == 'in') {
  			this.playerDiv.classList.add( 'fade-in' );
  			this.playerDiv.classList.remove( 'fade-out' );
  		}
  	};
  	...
  	AblePlayer.prototype.refreshControls = function(context = 'init', duration, elapsed) {
  		...
  		if ( context === 'timeline' || context === 'init' ) {
  			...
  			if (this.seekBar) {
  				...
  				if (!(this.seekBar.tracking)) {
  					...
  					updateLive = this.liveUpdatePending || this.seekBar.seekHead === AblePlayer.getActiveDOMElement();
  			...
  			if (typeof this.durationContainer !== 'undefined') {
  				if (this.useChapterTimes) {
  					this.durationContainer.textContent = this.formatSecondsAsColonTime(this.chapterDuration);
  				} else {
  					this.durationContainer.textContent = this.formatSecondsAsColonTime(this.duration);
  				}
  			}
  			if (typeof this.elapsedTimeContainer !== 'undefined') {
  				this.elapsedTimeContainer.textContent = this.formatSecondsAsColonTime(displayElapsed);
  			}
  			...
  			if (this.skin === 'legacy') {
  				...
  				if (this.seekBar) {
  					let seekbarSpan = this.seekBar.wrapperDiv.parentElement;
  					let controlWrapper = seekbarSpan.parentElement;
  					leftControls = seekbarSpan.previousElementSibling;
  					if (leftControls && !leftControls.matches('div.able-left-controls')) {
  						leftControls = null;
  					}
  					rightControls = leftControls ? leftControls.nextElementSibling : null;
  					if (rightControls && !rightControls.matches('div.able-right-controls')) {
  						rightControls = null;
  					}
  					widthUsed = leftControls ? leftControls.offsetWidth : 0;
  					if (rightControls) {
  						Array.from(rightControls.children).forEach(function (child) {
  							if (child.getAttribute('role') == 'button') {
  								widthUsed += child.offsetWidth + 5;
  							}
  						});
  					}
  					if (this.fullscreen) {
  						seekbarWidth = window.innerWidth - widthUsed;
  					} else {
  						...
  						seekbarWidth = controlWrapper.getBoundingClientRect().width - widthUsed - 10;
  					}
  		...
  		if (context === 'captions' || context == 'init') {
  			...
  			if (this.ccButton) {
  				...
  				if (captionsCount > 1) {
  					this.ccButton.setAttribute('aria-haspopup', 'true');
  					this.ccButton.setAttribute('aria-controls', this.mediaId + '-captions-menu');
  				}
  		...
  		if (context === 'fullscreen' || context == 'init'){
  			if (this.fullscreenButton) {
  				if (!this.fullscreen) {
  					this.fullscreenButton.setAttribute( 'aria-label', this.translate( 'enterFullScreen', 'Enter full screen' ) );
  					this.getIcon( this.fullscreenButton, 'fullscreen-expand' );
  				} else {
  					this.fullscreenButton.setAttribute('aria-label', this.translate( 'exitFullScreen', 'Exit full screen' ) );
  			...
  			if (typeof this.bigPlayButton !== 'undefined' && typeof this.seekBar !== 'undefined') {
  				...
  				if (this.paused && !this.seekBar.tracking) {
  					if (!this.hideBigPlayButton) {
  						this.bigPlayButton.style.display = '';
  						this.bigPlayButton.setAttribute('aria-hidden', 'false');
  					}
  				} else {
  					this.bigPlayButton.style.display = 'none';
  					this.bigPlayButton.setAttribute('aria-hidden', 'true');
  				}
  			}
  		}
  		...
  		if (context === 'transcript' || context == 'init'){
  			...
  			if (this.transcriptType) {
  				...
  				if (this.prefAutoScrollTranscript === 1) {
  					...
  					this.autoScrollTranscriptCheckbox.checked = true;
  				} else {
  					this.autoScrollTranscript = false;
  					this.autoScrollTranscriptCheckbox.checked = false;
  				}

  				// If transcript locked, scroll transcript to current highlight location.
  				if (this.autoScrollTranscript && this.currentHighlight) {
  					newTop = Math.floor(this.transcriptDiv.scrollTop +
  						this.currentHighlight[0].offsetTop -
  						(this.transcriptDiv.getBoundingClientRect().height / 2) +
  						(this.currentHighlight[0].getBoundingClientRect().height / 2));
  		...
  		if (context === 'init') {
  			...
  			if (this.chaptersButton) {
  				this.chaptersButton.setAttribute('aria-label', this.translate( 'chapters', 'Chapters' ));
  				this.chaptersButton.setAttribute('aria-haspopup', 'true');
  				this.chaptersButton.setAttribute('aria-controls', this.mediaId + '-chapters-menu');
  			}
  		}
  		...
  		if (context === 'timeline' || context === 'playpause' || context === 'init') {
  			...
  			if (this.stoppingYouTube) {
  				...
  				if (this.status.textContent !== this.translate( 'statusStopped', 'Stopped' ) ) {
  					this.status.textContent = this.translate( 'statusStopped', 'Stopped' );
  				}
  				...
  				this.getPlayerState().then(function(currentState) {
  					if (thisObj.status.textContent !== textByState[currentState] && !thisObj.seekBar.tracking) {
  					...
  					if (!thisObj.seekBar.tracking && !thisObj.stoppingYouTube) {
  						if (currentState === 'paused' || currentState === 'stopped' || currentState === 'ended') {
  							thisObj.playpauseButton.setAttribute('aria-label', thisObj.translate( 'play', 'Play' ) );
  							thisObj.getIcon( thisObj.playpauseButton, 'play' );
  						} else {
  							thisObj.playpauseButton.setAttribute('aria-label', thisObj.translate( 'pause', 'Pause' ) );
  		...
  		if (!this.fullscreen) {
  			...
  			var speedSpan = this.statusBarDiv.querySelector('span.able-speed');
  			if (this.statusBarDiv.getBoundingClientRect().width < statusBarWidthBreakpoint) {
  	...
  	AblePlayer.prototype.handleCaptionToggle = function() {
  		...
  		if (captions.length === 1) {
  			...
  			if (this.captionsPopup && this.captionsPopup.offsetParent !== null) {
  				this.captionsPopup.style.display = 'none';
  				this.hidingPopup = false;
  				this.ccButton.setAttribute('aria-expanded', 'false');
  				this.waitThenFocus(this.ccButton);
  				...
  				if (this.captionsPopup) {
  					this.captionsPopup.style.display = '';
  					this.ccButton.setAttribute('aria-expanded', 'true');

  					// Gives time to "register" expanded ccButton
  					setTimeout(function() {
  						thisObj.captionsPopup.style.top = (thisObj.ccButton.offsetTop - thisObj.captionsPopup.offsetHeight) + 'px';
  						thisObj.captionsPopup.style.left = thisObj.ccButton.offsetLeft + 'px';
  						// Place focus on the first button (even if another button is checked)
  						var capItems = Array.from(thisObj.captionsPopup.querySelectorAll('li'));
  						capItems.forEach(function (li) {
  							li.classList.remove('able-focus');
  						});
  						if (capItems.length) {
  							capItems[0].focus();
  							capItems[0].classList.add('able-focus');
  						}
  					}, 50);
  				}
  			}
  	...
  	AblePlayer.prototype.handleChapters = function () {
  		...
  		if (this.chaptersPopup.offsetParent !== null) {
  			this.chaptersPopup.style.display = 'none';
  			this.hidingPopup = false;
  			this.chaptersButton.setAttribute('aria-expanded', 'false');
  			this.chaptersButton.focus();
  			...
  			this.closePopups();
  			this.chaptersPopup.style.display = '';
  			this.chaptersButton.setAttribute('aria-expanded', 'true');
  			this.chaptersPopup.style.top = (this.chaptersButton.offsetTop - this.chaptersPopup.offsetHeight) + 'px';
  			this.chaptersPopup.style.left = this.chaptersButton.offsetLeft + 'px';

  			// Highlight the current chapter, if any chapters are checked
  			// Otherwise, place focus on the first chapter
  			var chapItems = Array.from(this.chaptersPopup.querySelectorAll('li'));
  			chapItems.forEach(function (li) {
  				li.classList.remove('able-focus');
  			});
  			var checkedChapter = this.chaptersPopup.querySelector('li[aria-checked="true"]');
  			if (checkedChapter) {
  				checkedChapter.focus();
  				checkedChapter.classList.add('able-focus');
  			} else if (chapItems.length) {
  				chapItems[0].classList.add('able-focus');
  				chapItems[0].setAttribute('aria-checked', 'true');
  				chapItems[0].focus();
  			}
  		}
  	};
  	...
  	AblePlayer.prototype.handleDescriptionToggle = function() {
  		...
  		if (typeof this.descDiv !== 'undefined') {
  			if (this.descDiv.offsetParent !== null) {
  				this.descDiv.style.display = 'none';
  			}
  	...
  	AblePlayer.prototype.handlePrefsClick = function(pref) {
  		...
  		if (this.prefsPopup.offsetParent !== null) {
  			this.prefsPopup.style.display = 'none';
  			this.prefsButton.setAttribute('aria-expanded', 'false');
  			// restore each menu item to original hidden state
  			Array.from(this.prefsPopup.querySelectorAll('li')).forEach(function (li) {
  				li.classList.remove('able-focus');
  				li.setAttribute('tabindex', '-1');
  			});
  			if (!this.showingPrefsDialog) {
  			...
  			this.closePopups();
  			this.prefsPopup.style.display = '';
  			this.prefsButton.setAttribute('aria-expanded', 'true');
  			...
  			setTimeout(function() {
  				prefsButtonPosition = { top: thisObj.prefsButton.offsetTop, left: thisObj.prefsButton.offsetLeft };
  				prefsMenuRight = thisObj.ableDiv.getBoundingClientRect().width - 5;
  				prefsMenuLeft = prefsMenuRight - thisObj.prefsPopup.getBoundingClientRect().width;
  				thisObj.prefsPopup.style.top = (prefsButtonPosition.top - thisObj.prefsPopup.offsetHeight) + 'px';
  				thisObj.prefsPopup.style.left = prefsMenuLeft + 'px';
  				// remove prior focus and set focus on first item; also change tabindex from -1 to 0
  				var prefItems = Array.from(thisObj.prefsPopup.querySelectorAll('li'));
  				prefItems.forEach(function (li) {
  					li.classList.remove('able-focus');
  					li.setAttribute('tabindex', '0');
  				});
  				if (prefItems.length) {
  					prefItems[0].focus();
  					prefItems[0].classList.add('able-focus');
  				}
  			}, 50);
  		}
  	};
  	...
  	AblePlayer.prototype.handleTranscriptToggle = function () {
  		var thisObj = this;
  		var visible = this.transcriptDiv.offsetParent !== null;
  		if ( visible ) {
  			...
  			if ( this.transcriptType === 'popup' ) {
  				this.transcriptButton.focus();
  				this.transcriptButton.classList.add('able-focus');
  			...
  			if ( this.transcriptType === 'popup' ) {
  				...
  				var firstTranscriptButton = this.transcriptArea.querySelector('button');
  				if (firstTranscriptButton) {
  					firstTranscriptButton.focus();
  				}
  				// wait briefly before resetting stopgap var
  				setTimeout(function() {
  	...
  	AblePlayer.prototype.handleSignToggle = function () {
  		...
  		var thisObj = this;
  		var visible = this.signWindow.offsetParent !== null;
  		if ( visible ) {
  			...
  			this.signButton.classList.add('able-focus');
  			// wait briefly before resetting stopgap var
  			// otherwise the keypress used to select 'Close' will trigger the transcript button
  			setTimeout(function() {
  			...
  			this.prefSign = 1;
  			this.focusNotClick = true;
  			var firstSignButton = this.signWindow.querySelector('button');
  			if (firstSignButton) {
  	...
  	AblePlayer.prototype.getIcon = function( button, id) {
  		...
  		var existingIcon = button.querySelector( 'svg#ableplayer-' + id );
  		// Avoid repainting icon if there's no change.
  		if ( existingIcon ) {
  			return;
  		}
  		Array.from(button.querySelectorAll('svg')).forEach(function (svg) {
  			svg.remove();
  		});
  	...
  	AblePlayer.prototype.toggleButtonState = function(button, isOn, onLabel, offLabel, ariaPressed = false, ariaExpanded = false) {
  		...
  		let buttonOff = ( button.classList.contains( 'buttonOff' ) ) ? true : false;
  		...
  		if (! isOn) {
  			...
  			button.setAttribute('aria-label', onLabel);
  			if ( ariaPressed ) {
  	...
  	AblePlayer.prototype.showAlert = function( msg, location = 'main' ) {
  		...
  		var alertSpan = alertBox.querySelector('span');
  		if (alertSpan) {
  			alertSpan.textContent = msg;
  		}
  		parentWindow.append(alertBox);
  	...
  	AblePlayer.prototype.resizePlayer = function (width, height) {
  		...
  		if (typeof width !== 'undefined' && typeof height !== 'undefined') {
  			...
  			if (this.playerWidth) {
  				...
  				newWidth = (this.player === 'html5') ? window.innerWidth : this.ableWrapper.getBoundingClientRect().width;
  				newHeight = Math.round(newWidth * this.aspectRatio);
  			}
  		} else if (this.fullscreen) {
  			this.ableWrapper.classList.add('fullscreen');
  			newWidth = window.innerWidth;
  			// the 5 pixel buffer is arbitrary, but results in a better fit for all browsers
  			newHeight = window.innerHeight - this.playerDiv.offsetHeight - 5;
  			this.positionCaptions('overlay');
  		} else { // not fullscreen, and not first time initializing player
  			this.ableWrapper.classList.remove('fullscreen');
  			if (this.player === 'html5') {
  				newWidth = (this.playerWidth) ? this.playerWidth : window.innerWidth;
  			} else {
  				newWidth = this.ableWrapper.getBoundingClientRect().width;
  			}
  			newHeight = Math.round(newWidth * this.aspectRatio);
  		...
  		if (this.player === 'youtube' || this.player === 'vimeo') {
  			...
  			if (this.playerWidth && this.playerHeight) {
  				if (this.fullscreen) {
  					...
  					if (iframe) {
  						iframe.style.maxWidth = this.playerWidth + 'px';
  						iframe.style.maxHeight = this.playerHeight + 'px';
  					}
  				}
  			}
  		} else if (this.player === 'html5') {
  			if (this.fullscreen) {
  				this.media.setAttribute('width', newWidth);
  				this.media.setAttribute('height', newHeight);
  				this.ableWrapper.style.width = newWidth + 'px';
  				this.ableWrapper.style.height = newHeight + 'px';
  			} else {
  					// No constraints. Let CSS handle the positioning.
  				this.media.removeAttribute('width');
  				this.media.removeAttribute('height');
  				this.ableWrapper.removeAttribute( 'style' );
  			}
  		}
  		// Resize captions
  		if (typeof this.captionsDiv !== 'undefined') {
  			...
  			this.captionsDiv.style.fontSize = captionSize;
  		}
  		this.refreshControls();
  	};
  	...
  	AblePlayer.prototype.retrieveOffscreenWindow = function( which, width, height ) {
  		...
  		var win, windowPos, windowTop, windowLeft, windowRight, windowWidth, windowBottom, windowHeight;

  		if (which == 'transcript') {
  			win = this.transcriptArea;
  		} else if (which == 'sign') {
  			win = this.signWindow;
  		}
  		windowWidth = win.getBoundingClientRect().width;
  		windowHeight = win.getBoundingClientRect().height;
  		windowPos = { top: win.offsetTop, left: win.offsetLeft };
  	...
  	AblePlayer.prototype.updateZIndex = function(which) {
  		...
  		if (typeof this.transcriptArea === 'undefined' || typeof this.signWindow === 'undefined' ) {
  			...
  			if (typeof this.transcriptArea !== 'undefined') {
  				transcriptZ = parseInt(window.getComputedStyle(this.transcriptArea).zIndex);
  				if (transcriptZ > defLowZ) {
  					// restore to the default
  					this.transcriptArea.style.zIndex = defLowZ;
  				}
  			} else if (typeof this.signWindow !== 'undefined') {
  				signZ = parseInt(window.getComputedStyle(this.signWindow).zIndex);
  				if (signZ > defHighZ) {
  					// restore to the default
  					this.signWindow.style.zIndex = defHighZ;
  				}
  			}
  			return false;
  		}
  		...
  		if (which === 'transcript') {
  			this.transcriptArea.style.zIndex = newHighZ;
  			this.signWindow.style.zIndex = newLowZ;
  		} else if (which === 'sign') {
  			this.signWindow.style.zIndex = newHighZ;
  			this.transcriptArea.style.zIndex = newLowZ;
  		}
  	};
  				...
  				if (this.sampleText) {
  					for (i = 0; i < this.sampleText.length; i++) {
  						if (this.sampleText[i].lang === this.selectedDescriptions.language) {
  							...
  							this.sampleDescDiv.innerHTML = this.currentSampleText;
  						}
  					}
  				}
  			}
  		...
  		document.querySelectorAll('.able-playlist').forEach(function(playlistEl) {
  			if (thisObj.getData(playlistEl, 'player') === thisObj.mediaId) {
  				...
  				playlistEl.querySelectorAll('li[data-youtube-id]').forEach(function(li) {
  					var youTubeId = purify.sanitize( li.getAttribute('data-youtube-id') );
  ...
  function addSignFunctions(AblePlayer) {
  	AblePlayer.prototype.initSignLanguage = function() {
  		...
  		if ( ! this.isIOS() && ( hasLocalSrc || hasRemoteSrc || hasRemoteSource ) && ( this.player === 'html5' || this.player === 'youtube' ) ) {
  			...
  			let ytSignSrc = this.youTubeSignId ?? purify.sanitize( this.sources[0].getAttribute('data-youtube-sign-src') );
  			let signSrc = purify.sanitize( this.sources[0].getAttribute('data-sign-src') );
  			let signVideo = purify.sanitize( this.getData(this.media, 'youtube-sign-src') );
  ...
  var rewind$j = "Endarrere";
  var forward$j = "Endavant";
  ...
  var prefIntroDescriptionNone$j = "L'actual vídeo no disposa d'audiodescripció en cap format.";
  ...
  var faster$i = "Rychleji";
  var slower$i = "Pomaleji";
  var play$i = "Spustit";
  var pause$i = "Pauza";
  var restart$i = "Přehrát od začátku";
  var prevTrack$i = "Předchozí stopa";
  var nextTrack$i = "Další stopa";
  var rewind$i = "Přetočit vzad";
  var forward$i = "Přetočit vpřed";
  ...
  var prefIntroDescriptionNone$i = "Aktuální video nemá žádný zvukový popis v žádném formátu.";
  ...
  var faster$h = "Hurtigere";
  var slower$h = "Langsommere";
  var play$h = "Afspil";
  var pause$h = "Pause";
  var restart$h = "Genstart";
  var prevTrack$h = "Forrige spor";
  var nextTrack$h = "Næste spor";
  var rewind$h = "Spol tilbage";
  var forward$h = "Spol frem";
  ...
  var prefIntroDescriptionNone$h = "Nuværende video har ingen synstolkning i noget format.";
  ...
  var vtsHeading$h = "Video Transcript Sorter";
  var vtsInstructions1$h = "Use the Video Transcript Sorter to modify text tracks:";
  var vtsInstructions2$h = "Reorder chapters, descriptions, captions, and/or subtitles so they appear in the proper sequence in Able Player's auto-generated transcript.";
  var vtsInstructions3$h = "Modify content or start/end times (all are directly editable within the table).";
  var vtsInstructions4$h = "Add new content, such as chapters or descriptions.";
  var vtsInstructions5$h = "After editing, click the \"Save Changes\" button to generate new content for all relevant timed text files. The new text can be copied and pasted into new WebVTT files.";
  var vtsSelectLanguage$h = "Select a language";
  var vtsSave$h = "Generate new .vtt content";
  var vtsReturn$h = "Return to Editor";
  var vtsCancel$h = "Cancelling saving. Any edits you made have been restored in the VTS table.";
  var vtsRow$h = "Row";
  var vtsKind$h = "Kind";
  var vtsStart$h = "Start";
  var vtsEnd$h = "End";
  var vtsContent$h = "Content";
  var vtsActions$h = "Actions";
  var vtsNewRow$h = "A new row %1 has been inserted.";
  var vtsDeletedRow$h = "Row %1 has been deleted.";
  var vtsMovedRow$h = "Row %1 has been moved %2 and is now Row %3.";
  var da = {
  ...
  var faster$g = "Schneller";
  var slower$g = "Langsamer";
  ...
  var prefIntroDescriptionNone$g = "Das aktuelle Video hat keine Audiobeschreibung.";
  ...
  var prevTrack$f = "Previous track";
  var nextTrack$f = "Next track";
  var rewind$f = "Rewind";
  var forward$f = "Forward";
  var captions$f = "Captions";
  var showCaptions$f = "Show captions";
  var hideCaptions$f = "Hide captions";
  var captionsOff$f = "Captions off";
  var showTranscript$f = "Show transcript";
  var hideTranscript$f = "Hide transcript";
  var turnOnDescriptions$f = "Turn on descriptions";
  var turnOffDescriptions$f = "Turn off descriptions";
  var chapters$f = "Chapters";
  var language$f = "Language";
  var sign$f = "Sign language";
  var showSign$f = "Show sign language";
  var hideSign$f = "Hide sign language";
  var seekbarLabel$f = "timeline";
  var mute$f = "Mute";
  var unmute$f = "Unmute";
  var volume$f = "Volume";
  var volumeUpDown$f = "Volume up down";
  var preferences$f = "Preferences";
  var enterFullScreen$f = "Enter full screen";
  var exitFullScreen$f = "Exit full screen";
  var speed$f = "Speed";
  var spacebar$f = "spacebar";
  var transcriptTitle$f = "Transcript";
  var lyricsTitle$f = "Lyrics";
  var autoScroll$f = "Auto scroll";
  var statusPlaying$f = "Playing";
  var statusPaused$f = "Paused";
  var statusStopped$f = "Stopped";
  var statusBuffering$f = "Buffering";
  var statusEnd$f = "End of track";
  var selectedTrack$e = "Selected Track";
  var alertDescribedVersion$f = "Using the audio described version of this video";
  var alertNonDescribedVersion$f = "Using the non-described version of this video";
  var prefMenuCaptions$f = "Captions";
  var prefVoicedCaptions$f = "Spoken Captions";
  var prefMenuDescriptions$f = "Descriptions";
  var prefMenuKeyboard$f = "Keyboard";
  var prefMenuTranscript$f = "Transcript";
  var prefTitleCaptions$f = "Captions Preferences";
  var prefTitleDescriptions$f = "Audio Description Preferences";
  var prefTitleKeyboard$f = "Keyboard Preferences";
  var prefTitleTranscript$e = "Transcript Preferences";
  var prefIntroDescription1$e = "This media player supports audio description in two ways: ";
  var prefDescription1$f = "The current video has an alternative described version and text-based description, announced by screen reader.";
  var prefDescription2$f = "The current video has text-based description.";
  var prefDescription3$f = "The current video has an alternative described version.";
  var prefIntroDescriptionNone$f = "The current video has no audio description in either format.";
  var prefIntroDescription3$f = "Use the following form to set your preferences related to text-based audio description.";
  var prefIntroDescription4$f = "After you save your settings, audio description can be toggled on/off using the Description button.";
  var prefIntroKeyboard1$f = "The media player on this web page can be operated from anywhere on the page using keyboard shortcuts (see below for a list).";
  var prefIntroKeyboard2$f = "Modifier keys (Shift, Alt, and Control) can be assigned below.";
  var prefIntroKeyboard3$f = "NOTE: Some key combinations might conflict with keys used by your browser and/or other software applications. Try various combinations of modifier keys to find one that works for you.";
  var prefHeadingKeyboard1$f = "Modifier keys used for shortcuts";
  var prefHeadingKeyboard2$f = "Current keyboard shortcuts";
  var prefHeadingDescription$f = "Audio description";
  var prefHeadingTextDescription$f = "Text-based audio description";
  var prefAltKey$f = "Alt";
  var prefCtrlKey$f = "Control";
  var prefShiftKey$f = "Shift";
  var prefNoKeyShortcuts$f = "Disable keyboard shortcuts";
  var escapeKey$f = "Escape";
  var escapeKeyFunction$f = "Close current dialog or popup menu";
  var prefDescPause$f = "Automatically pause video when description starts";
  var prefDescVisible$f = "Make description visible";
  var prefDescVoice$f = "Voice";
  var prefDescRate$f = "Spoken Description Rate";
  var prefCaptionRate$f = "Spoken Caption Rate";
  var prefDescPitch$f = "Pitch";
  var prefDescPitch1$f = "Very low";
  var prefDescPitch2$f = "Low";
  var prefDescPitch3$f = "Default";
  var prefDescPitch4$f = "High";
  var prefDescPitch5$f = "Very high";
  var sampleDescriptionText$f = "Adjust settings to hear this sample text.";
  var prefHighlight$f = "Highlight transcript as media plays";
  var prefTabbable$f = "Keyboard-enable transcript";
  var prefCaptionsFont$f = "Font";
  var prefCaptionsColor$f = "Text Color";
  var prefCaptionsBGColor$f = "Background";
  var prefCaptionsSize$f = "Font Size";
  var prefCaptionsOpacity$f = "Opacity";
  var prefCaptionsStyle$f = "Style";
  var serif$f = "serif";
  var sans$f = "sans-serif";
  var cursive$f = "cursive";
  var fantasy$f = "fantasy";
  var monospace$f = "monospace";
  var white$f = "white";
  var yellow$f = "yellow";
  var green$f = "green";
  var cyan$e = "cyan";
  ...
  var red$f = "red";
  var black$f = "black";
  var transparent$f = "transparent";
  var solid$f = "solid";
  var captionsStylePopOn$f = "Pop-on";
  var captionsStyleRollUp$f = "Roll-up";
  var prefCaptionsPosition$f = "Position";
  var captionsPositionOverlay$f = "Overlay";
  var captionsPositionBelow$f = "Below video";
  var sampleCaptionText$f = "Sample caption text";
  var prefSuccess$f = "Your changes have been saved.";
  var prefNoChange$f = "You didn't make any changes.";
  var save$f = "Save";
  var cancel$f = "Cancel";
  var dismissButton$f = "Dismiss";
  var windowButtonLabel$f = "Window options";
  var windowMove$f = "Move";
  ...
  var windowMoveAlert$f = "Drag or use arrow keys to move the window; Enter to stop";
  var windowResize$f = "Resize";
  var windowResizeHeading$f = "Resize Window";
  var closeButtonLabel$f = "Close";
  var width$f = "Width";
  var height$f = "Height";
  ...
  var playerHeading$e = "Media player";
  ...
  var play$e = "Play";
  var pause$e = "Pausa";
  var restart$e = "Reiniciar";
  var prevTrack$e = "Pista Anterior";
  var nextTrack$e = "Siguiente Pista";
  var rewind$e = "Rebobinar";
  var forward$e = "Adelantar";
  var captions$e = "Subtítulos";
  var showCaptions$e = "Mostrar subtítulos";
  var hideCaptions$e = "Ocultar subtítulos";
  var captionsOff$e = "Sin subtítulos";
  var showTranscript$e = "Mostrar transcripción";
  var hideTranscript$e = "Ocultar transcripción";
  var turnOnDescriptions$e = "Habilitar descripciones";
  var turnOffDescriptions$e = "Deshabilitar descripciones";
  var chapters$e = "Capítulos";
  var language$e = "Idioma";
  var sign$e = "Lengua de señas";
  var showSign$e = "Mostrar lengua de señas";
  var hideSign$e = "Ocultar lengua de señas";
  var seekbarLabel$e = "timeline";
  var mute$e = "Silenciar";
  var unmute$e = "Habilitar sonido";
  var volume$e = "Volumen";
  var volumeUpDown$e = "Bajar sonido";
  var preferences$e = "Preferencias";
  var enterFullScreen$e = "Ver a pantalla completa";
  var exitFullScreen$e = "Salir de pantalla completa";
  var speed$e = "Velocidad";
  var spacebar$e = "Barra espaciadora";
  var transcriptTitle$e = "Transcript";
  var lyricsTitle$e = "Letra";
  var autoScroll$e = "Desplazamiento automático";
  var statusPlaying$e = "Reproduciendo";
  var statusPaused$e = "Pausado";
  var statusStopped$e = "Detenido";
  var statusBuffering$e = "Almacenando";
  var statusEnd$e = "Fin de pista";
  var selectedTrack$d = "Pista seleccionada";
  var alertDescribedVersion$e = "Utilizando la versión audiodescrita del vídeo";
  var alertNonDescribedVersion$e = "Utilizando la versión no descrita de este vídeo";
  var prefMenuCaptions$e = "Subtítulos";
  var prefVoicedCaptions$e = "Spoken Captions";
  var prefMenuDescriptions$e = "Descripciones";
  var prefMenuKeyboard$e = "Teclado";
  var prefMenuTranscript$e = "Transcripción";
  var prefTitleCaptions$e = "Preferencias de subtítulos";
  var prefTitleDescriptions$e = "Preferencias de audiodescripción";
  var prefTitleKeyboard$e = "Preferencias de teclado";
  var prefTitleTranscript$d = "Preferencias de transcripción";
  var prefIntroDescription1$d = "Este reproductor soporta la audiodescripción de dos maneras: ";
  var prefDescription1$e = "El vídeo actual tiene una versión alternativa con descripción, descripción en texto.";
  var prefDescription2$e = "El vídeo actual tiene versión alternativa del vídeo, descrito.";
  var prefDescription3$e = "El vídeo actual tiene descripción en texto, leída por el lector de pantalla.";
  var prefIntroDescriptionNone$e = "El vídeo actual no tiene audiodescripción de ninguna manera.";
  var prefIntroDescription3$e = "Utilice el siguiente formulario para establecer sus preferencias en cuanto a la audiodescripción en texto.";
  var prefIntroDescription4$e = "Una vez guardadas sus preferencias, la audiodescripción puede habilitarse o deshabilitarse mediante el botón Descripción.";
  var prefIntroKeyboard1$e = "El reproductor en esta página puede manejarse desde cualquier parte de la página utilizando los atajos de teclado (vea la lista más abajo).";
  var prefIntroKeyboard2$e = "Las teclas modificadoras (Mayúsculas, Alt, Control) pueden definirse más abajo.";
  var prefIntroKeyboard3$e = "NOTA: Algunas combinaciones de teclas pueden entrar en conflicto con teclas utilizadas por su navegador y/o otras aplicaciones. Intente varias combinaciones de teclas modificadoras para encontrar la que funciona bien en su caso.";
  var prefHeadingKeyboard1$e = "Teclas modificadoras utilizadas para atajos de teclado";
  var prefHeadingKeyboard2$e = "Atajos de teclado definidos actualmente";
  var prefHeadingDescription$e = "Audiodescrita";
  var prefHeadingTextDescription$e = "Audiodescrita en texto";
  var prefAltKey$e = "Alt";
  var prefCtrlKey$e = "Control";
  var prefShiftKey$e = "Mayúsculas";
  var prefNoKeyShortcuts$e = "Disable keyboard shortcuts";
  var escapeKey$e = "Escape";
  var escapeKeyFunction$e = "Cerrar el cuadro de diálogo o menú contextual";
  var prefDescPause$e = "Pausar automáticamente el video cuando arranque una descripción";
  var prefDescVisible$e = "Hacer visibles las descripciones en texto si se están usando";
  var prefDescVoice$e = "Voz";
  var prefDescRate$e = "Spoken Description Rate";
  var prefCaptionRate$e = "Spoken Caption Rate";
  var prefDescPitch$e = "Tono";
  var prefDescPitch1$e = "Muy grave";
  var prefDescPitch2$e = "Grave";
  var prefDescPitch3$e = "Normal";
  var prefDescPitch4$e = "Aguda";
  var prefDescPitch5$e = "Muy aguda";
  var sampleDescriptionText$e = "Ajuste la configuración para escuchar este texto de muestra";
  var prefHighlight$e = "Resalte la transcripción a medida que avance el contenido";
  var prefTabbable$e = "Transcripción manejable por teclado";
  var prefCaptionsFont$e = "Fuente";
  var prefCaptionsColor$e = "Color del texto";
  var prefCaptionsBGColor$e = "Fondo";
  var prefCaptionsSize$e = "Tamaño de Fuente";
  var prefCaptionsOpacity$e = "Opacidad";
  var prefCaptionsStyle$e = "Estilo";
  var serif$e = "serif";
  var sans$e = "sans-serif";
  var cursive$e = "cursiva";
  var fantasy$e = "fantasía";
  var monospace$e = "mono espaciada";
  var white$e = "blanco";
  var yellow$e = "amarillo";
  var green$e = "verde";
  var cyan$d = "cyan";
  var blue$e = "azul";
  ...
  var red$e = "rojo";
  var black$e = "negro";
  var transparent$e = "transparente";
  var solid$e = "sólido";
  var captionsStylePopOn$e = "Pop-on";
  var captionsStyleRollUp$e = "Roll-up";
  var prefCaptionsPosition$e = "Posición";
  var captionsPositionOverlay$e = "Cubrir";
  var captionsPositionBelow$e = "Debajo del vídeo";
  var sampleCaptionText$e = "Texto de ejemplo de subtítulo";
  var prefSuccess$e = "Los cambios han sido guardados.";
  var prefNoChange$e = "No se ha hecho ningún cambio.";
  var save$e = "Guardar";
  var cancel$e = "Cancelar";
  var dismissButton$e = "Dismiss";
  var windowButtonLabel$e = "Opciones en Windows";
  var windowMove$e = "Mover";
  ...
  var windowMoveAlert$e = "Arrastre o use las teclas de flecha para mover la ventana, pulse Enter para parar.";
  var windowResize$e = "Redimensionar";
  var windowResizeHeading$e = "Redimensionar la ventana con el intérprete";
  var closeButtonLabel$e = "Cerrar";
  var width$e = "Ancho";
  var height$e = "Alto";
  var resultsSummary1$e = "You searched for:";
  var resultsSummary2$e = "Found %1 matching items.";
  var resultsSummary3$e = "Click the time associated with any item to play the video from that point.";
  var noResultsFound$e = "No results found.";
  var searchButtonLabel$e = "Play at %1";
  var hour$e = "hour";
  var minute$e = "minute";
  var second$e = "second";
  var hours$e = "hours";
  ...
  var playerHeading$d = "Lecteur multimédia";
  var audioPlayer$d = "Audio player";
  var videoPlayer$d = "Video player";
  var faster$d = "Plus rapidement";
  var slower$d = "Plus lentement";
  var play$d = "Lecture";
  var pause$d = "Pause";
  var restart$d = "Redémarrer";
  var prevTrack$d = "Piste Précédente";
  var nextTrack$d = "Piste Suivante";
  var rewind$d = "Reculer";
  var forward$d = "Avancer";
  var captions$d = "Sous-titres";
  var showCaptions$d = "Afficher les sous-titres";
  var hideCaptions$d = "Masquer les sous-titres";
  var captionsOff$d = "Sous-titres désactivés";
  var showTranscript$d = "Afficher la transcription";
  var hideTranscript$d = "Masquer la transcription";
  var turnOnDescriptions$d = "Activer les descriptions";
  var turnOffDescriptions$d = "Désactiver les descriptions";
  var chapters$d = "Chapitres";
  var language$d = "Langue";
  var sign$d = "Langage gestuel";
  var showSign$d = "Afficher le langage gestuel";
  var hideSign$d = "Masque le langage gestuel";
  var seekbarLabel$d = "timeline";
  var mute$d = "Son désactivé";
  var unmute$d = "Son activé";
  var volume$d = "Volume";
  var volumeUpDown$d = "Monter baisser le volume";
  var preferences$d = "Préférences";
  var enterFullScreen$d = "Activer le mode plein écran";
  var exitFullScreen$d = "Quitter le mode plein écran";
  var speed$d = "Vitesse";
  var spacebar$d = "barre d’espacement";
  var transcriptTitle$d = "Transcription";
  var lyricsTitle$d = "Paroles";
  var autoScroll$d = "Défilement automatique";
  var statusPlaying$d = "Lecture en cours";
  var statusPaused$d = "Lecture sur pause";
  var statusStopped$d = "Lecture interrompue";
  var statusBuffering$d = "Tamponnage";
  var statusEnd$d = "Fin de la piste";
  var selectedTrack$c = "Piste choisie";
  var alertDescribedVersion$d = "Utilisation de la version avec description sonore de cette vidéo";
  var alertNonDescribedVersion$d = "Utilisation de la version non décrite de cette vidéo";
  var prefMenuCaptions$d = "Sous-titres";
  var prefVoicedCaptions$d = "Spoken Captions";
  var prefMenuDescriptions$d = "Descriptions";
  var prefMenuKeyboard$d = "Clavier";
  var prefMenuTranscript$d = "Transcription";
  var prefTitleCaptions$d = "Préférences liées au sous-titrage";
  var prefTitleDescriptions$d = "Préférences liées aux descriptions sonores";
  var prefTitleKeyboard$d = "Préférences liées au clavier";
  var prefTitleTranscript$c = "Préférence liées à la transcription";
  var prefIntroDescription1$c = "Ce lecteur multimédia permet d’entendre les descriptions sonores de deux façons:";
  var prefDescription1$d = "Il y a une version autre version avec description, description textuelle.";
  var prefDescription2$d = "Il y a une version autre version de la vidéo avec description.";
  var prefDescription3$d = "Il y a une version description textuelle, lue à l’aide d’un lecteur d’écran.";
  var prefIntroDescriptionNone$d = "Il n’y a pas de version avec description sonore (dans ni l’un ni l’autre des formats) de la présente vidéo.";
  var prefIntroDescription3$d = "Utilisez le formulaire suivant pour établir vos préférences liées aux descriptions sonores textuelle.";
  var prefIntroDescription4$d = "Après avoir enregistré vos préférences, vous pouvez activer ou désactiver la description sonore avec le bouton Description.";
  var prefIntroKeyboard1$d = "Le lecteur multimédia de cette page Web peut être utilisé à partir de n’importe quel endroit sur la page avec des raccourcis du clavier (voir la liste ci-dessous).";
  var prefIntroKeyboard2$d = "Des rôles peuvent être assignés aux touches de modification (Shift, Alt, Ctrl) ci-dessous.";
  var prefIntroKeyboard3$d = "Certaines combinaisons de touches pourraient entrer en conflit avec les touches utilisées par votre navigateur ou autres applications logicielles. Essayez diverses combinaisons de touches de modification pour en trouver qui fonctionnent pour vous.";
  var prefHeadingKeyboard1$d = "Touches de modification utilisées pour des raccourcis";
  var prefHeadingKeyboard2$d = "Raccourcis du clavier assignés actuellement";
  var prefHeadingDescription$d = "Description sonore";
  var prefHeadingTextDescription$d = "Description sonore textuelle";
  var prefAltKey$d = "Alt";
  var prefCtrlKey$d = "Ctrl";
  var prefShiftKey$d = "Shift";
  var prefNoKeyShortcuts$d = "Disable keyboard shortcuts";
  var escapeKey$d = "Esc";
  var escapeKeyFunction$d = "Fermer la fenêtre de dialogue ou le menu contextuel";
  var prefDescPause$d = "Mettre la vidéo en pause automatiquement quand la description commence";
  var prefDescVisible$d = "Affichez la description";
  var prefDescVoice$d = "Voix";
  var prefDescRate$d = "Spoken Description Rate";
  var prefCaptionRate$d = "Spoken Caption Rate";
  var prefDescPitch$d = "Volume";
  var prefDescPitch1$d = "Très faible";
  var prefDescPitch2$d = "Faible";
  var prefDescPitch3$d = "Par défaut";
  var prefDescPitch4$d = "Fort";
  var prefDescPitch5$d = "Très fort";
  var sampleDescriptionText$d = "Régler les paramètres afin d'entendre cet extrait de texte";
  var prefHighlight$d = "Surligner la transcription pendant la lecture";
  var prefTabbable$d = "Transcription activée par clavier";
  var prefCaptionsFont$d = "Police de caractères";
  var prefCaptionsColor$d = "Couleur du texte";
  var prefCaptionsBGColor$d = "Arrière-plan";
  var prefCaptionsSize$d = "Taille de la police";
  var prefCaptionsOpacity$d = "Opacité";
  var prefCaptionsStyle$d = "Style";
  var serif$d = "avec empattement";
  var sans$d = "sans empattement";
  var cursive$d = "écriture cursive";
  var fantasy$d = "écriture de fantaisie";
  var monospace$d = "à taille fixe";
  var white$d = "blanc";
  var yellow$d = "jaune";
  var green$d = "vert";
  var cyan$c = "cyan";
  ...
  var red$d = "rouge";
  var black$d = "noir";
  var transparent$d = "transparent";
  var solid$d = "solide";
  var captionsStylePopOn$d = "Pop-on";
  var captionsStyleRollUp$d = "Roll-up";
  var prefCaptionsPosition$d = "Position";
  var captionsPositionOverlay$d = "Superposés";
  var captionsPositionBelow$d = "Sous la vidéo";
  var sampleCaptionText$d = "Échantillon de sous-titre";
  var prefSuccess$d = "Vos changements ont été enregistrés.";
  var prefNoChange$d = "Vous n’avez pas fait de changements.";
  var save$d = "Enregistrer";
  var cancel$d = "Annuler";
  var dismissButton$d = "Dismiss";
  var windowButtonLabel$d = "Options de fenêtre";
  var windowMove$d = "Déplacer";
  ...
  var windowMoveAlert$d = "Faites glisser avec la souris ou utilisez les touches fléchées pour déplacer la fenêtre; appuyez sur « Enter » pour arrêter.";
  var windowResize$d = "Redimensionner";
  var windowResizeHeading$d = "Redimensionner la fenêtre de l’interprète";
  var closeButtonLabel$d = "Fermer";
  var width$d = "Largeur";
  var height$d = "Hauteur";
  var resultsSummary1$d = "Vos résultats de recherche";
  var resultsSummary2$d = "Trouvé %1 éléments correspondants.";
  var resultsSummary3$d = "Cliquez sur le temps associé à n'importe quel élément pour lire la vidéo à partir de ce point.";
  var noResultsFound$d = "Aucun résultat trouvé.";
  var searchButtonLabel$d = "Lecture à %1";
  var hour$d = "heure";
  var minute$d = "minute";
  var second$d = "seconde";
  var hours$d = "heures";
  ...
  var playerHeading$c = "נגן מדיה";
  var audioPlayer$c = "Audio player";
  var videoPlayer$c = "Video player";
  var faster$c = "מהר יותר";
  var slower$c = "לאט יותר";
  var play$c = "נגן";
  var pause$c = "הפסקה";
  var restart$c = "התחלה מחדש";
  var prevTrack$c = "המסלול הקודם";
  var nextTrack$c = "המסלול הבא";
  var rewind$c = "חזרה";
  var forward$c = "קדימה";
  var captions$c = "כיתובים";
  var showCaptions$c = "הצגת כיתובים";
  var hideCaptions$c = "הסתרת כיתובים";
  var captionsOff$c = "ללא כיתובים";
  var showTranscript$c = "הצג תמליל";
  var hideTranscript$c = "הסתר תמליל";
  var turnOnDescriptions$c = "הפעל תמליל";
  var turnOffDescriptions$c = "סגור תמליל";
  var chapters$c = "פרקים";
  var language$c = "שפה";
  var sign$c = "שפת סימנים";
  var showSign$c = "הצג שפת סימנים";
  var hideSign$c = "הסתר שפת סימנים";
  var seekbarLabel$c = "ציר זמן";
  var mute$c = "השתקה";
  var unmute$c = "הפסקת השתקה";
  var volume$c = "עצמה";
  var volumeUpDown$c = "Volume up down";
  var preferences$c = "העדפות";
  var enterFullScreen$c = "מעבר למסך מלא";
  var exitFullScreen$c = "יציאה ממסך מלא";
  var speed$c = "מהירות";
  var spacebar$c = "מקש הרווח";
  var transcriptTitle$c = "תמליל";
  var lyricsTitle$c = "מילים";
  var autoScroll$c = "גלילה אוטומטית";
  var statusPlaying$c = "מנגן";
  var statusPaused$c = "מופסק";
  var statusStopped$c = "נעצר";
  var statusBuffering$c = "חציצה";
  var statusEnd$c = "סוף המסלול";
  var selectTrack = "מסלול נבחר";
  var alertDescribedVersion$c = "שימוש בגירסת אודיו המתוארת של סרטון זה";
  var alertNonDescribedVersion$c = "שימוש בגרסה הלא מתוארת של סרטון זה";
  var prefMenuCaptions$c = "כיתובים";
  var prefVoicedCaptions$c = "Spoken Captions";
  var prefMenuDescriptions$c = "תיאורים";
  var prefMenuKeyboard$c = "מקלדת";
  var prefMenuTranscript$c = "תמליל";
  var prefTitleCaptions$c = "העדפות כיתובים";
  var prefTitleDescriptions$c = "העדפות תיאור שמע";
  var prefTitleKeyboard$c = "העדפות מקלדת";
  var PrefTitleTranscript = "העדפות תמליל";
  var prefIntrodescription1 = "נגן המדיה תומך בתיאור השמע בשתי דרכים:";
  var prefDescription1$c = "הווידאו הנוכחי יש גרסה חלופית המתוארתתיאור מבוסס טקסט";
  var prefDescription2$c = "הווידאו הנוכחי ישגרסה חלופית המתוארת של וידאו";
  var prefDescription3$c = "הווידאו הנוכחי יש תיאור מבוסס טקסט, הודיעה על ידי קורא מסך";
  var prefIntroDescriptionNone$c = "לסרטון הנוכחי אין תיאור שמע בתבנית.";
  var prefIntroDescription3$c = "השתמש בטופס הבא כדי לקבוע את ההעדפות שלך הקשורות לתיאור שמע.";
  var prefIntroDescription4$c = "לאחר שתשמור את ההגדרות שלך, תיאור השמע יכול להיות מוחל על / כיבוי באמצעות לחצן תיאור.";
  var prefIntroKeyboard1$c = "נגן המדיה בדף אינטרנט זה יכול להיות מופעל מכל מקום בדף באמצעות קיצורי מקשים (ראה להלן רשימה).";
  var prefIntroKeyboard2$c = "ניתן לשנות את מקשי השינוי (Shift, Alt ו- Control).";
  var prefIntroKeyboard3$c = "הערה: ייתכן שחלק מצירופי המקשים מתנגשים עם המקשים המשמשים את הדפדפן ו / או יישומי תוכנה אחרים, נסה שילובים שונים של מפתחות שינוי כדי למצוא אחד המתאים לך.";
  var prefHeadingKeyboard1$c = "מקשי משנה המשמשים לקיצורי דרך";
  var prefHeadingKeyboard2$c = "קיצורי מקשים נוכחיים";
  var prefHeadingDescription$c = "תיאור שמע";
  var prefHeadingTextDescription$c = "תיאור מבוסס טקסט מבוסס טקסט";
  ...
  var escapeKeyFunction$c = "סגור את תיבת הדו שיח הנוכחית או תפריט קופץ";
  var prefDescPause$c = "השהה את הווידאו באופן אוטומטי כאשר התיאור מתחיל";
  var prefDescVisible$c = "הפוך תיאור גלוי";
  var prefDescVoice$c = "Voice";
  var prefDescRate$c = "Spoken Description Rate";
  var prefCaptionRate$c = "Spoken Caption Rate";
  var prefDescPitch$c = "Pitch";
  var prefDescPitch1$c = "Very low";
  var prefDescPitch2$c = "Low";
  var prefDescPitch3$c = "Default";
  var prefDescPitch4$c = "High";
  var prefDescPitch5$c = "Very high";
  var sampleDescriptionText$c = "Adjust settings to hear this sample text.";
  var prefHighlight$c = "הדגש תמליל כמו משחק מדיה";
  var prefTabbable$c = "הפעל תמליל";
  var prefCaptionsFont$c = "גופן";
  var prefCaptionsColor$c = "צבע טקסט";
  var prefCaptionsBGColor$c = "רקע";
  var prefCaptionsSize$c = "גודל גופן";
  var prefCaptionsOpacity$c = "אטימות";
  var prefCaptionsStyle$c = "סגנון";
  var serif$c = "serif";
  var sans$c = "sans-serif";
  var cursive$c = "cursive";
  var fantasy$c = "fantasy";
  var monospace$c = "monospace";
  var white$c = "לבן";
  var yellow$c = "צהוב";
  var green$c = "ירוק";
  var cyan$b = "טורקיז";
  ...
  var red$c = "אדום";
  var black$c = "שחור";
  var transparent$c = "שקוף";
  var solid$c = "מלא";
  var captionsStylePopOn$c = "Pop-on";
  var captionsStyleRollUp$c = "Roll-up";
  var prefCaptionsPosition$c = "מיקום";
  var captionsPositionOverlay$c = "Overlay";
  var captionsPositionBelow$c = "מתחת לוידאו";
  var sampleCaptionText$c = "דוגמת טקסט כיתוביות";
  var prefSuccess$c = "השינויים שלכם נשמרו.";
  var prefNoChange$c = "לא בצעתם כל שינוי.";
  var save$c = "שמירה";
  var cancel$c = "ביטול";
  var dismissButton$c = "Dismiss";
  var windowButtonLabel$c = "אפשרויות חלון";
  var windowMove$c = "הזזה";
  ...
  var windowMoveAlert$c = "גרור או השתמשו מקשי החצים להזזת החלון; כפתור Enter להפסקה";
  var windowResize$c = "שיוי גודל";
  var windowResizeHeading$c = "שינוי גודל חלון";
  var closeButtonLabel$c = "סגירת";
  var width$c = "רוחב";
  var height$c = "גובה";
  var resultsSummary1$c = "You searched for:";
  var resultsSummary2$c = "Found %1 matching items.";
  var resultsSummary3$c = "Click the time associated with any item to play the video from that point.";
  var noResultsFound$c = "No results found.";
  var searchButtonLabel$c = "Play at %1";
  var hour$c = "hour";
  var minute$c = "minute";
  var second$c = "second";
  var hours$c = "hours";
  ...
  var playerHeading$b = "Pemutar media";
  var audioPlayer$b = "Audio player";
  var videoPlayer$b = "Video player";
  var faster$b = "Percepat";
  var slower$b = "Perlambat";
  var play$b = "Mulai";
  var pause$b = "Jeda";
  var restart$b = "Ulangi";
  var prevTrack$b = "Trek sebelumnya";
  var nextTrack$b = "Trek berikutnya";
  var rewind$b = "Mundur";
  var forward$b = "Maju";
  var captions$b = "Takarir";
  var showCaptions$b = "Tampilkan takarir";
  var hideCaptions$b = "Sembunyikan takarir";
  var captionsOff$b = "Tanpa takarir";
  var showTranscript$b = "Tampilkan transkripsi";
  var hideTranscript$b = "Sembunyikan transkripsi";
  var turnOnDescriptions$b = "Nyalakan deskripsi";
  var turnOffDescriptions$b = "Matikan deskripsi";
  var chapters$b = "Bagian";
  var language$b = "Bahasa";
  var sign$b = "Bahasa isyarat";
  var showSign$b = "Tampilkan bahasa isyarat";
  var hideSign$b = "Sembunyikan bahasa isyarat";
  var seekbarLabel$b = "lini masa";
  var mute$b = "Matikan Suara";
  var unmute$b = "Nyalakan Suara";
  var volume$b = "Volume";
  var volumeUpDown$b = "Naik turun volume";
  var preferences$b = "Preferensi";
  var enterFullScreen$b = "Masuk ke mode layar penuh";
  var exitFullScreen$b = "Keluar dari mode layar penuh";
  var speed$b = "Kecepatan";
  var spacebar$b = "tombol spasi";
  var transcriptTitle$b = "Transkripsi";
  var lyricsTitle$b = "Lirik";
  var autoScroll$b = "Gulir otomatis";
  var statusPlaying$b = "Memutar";
  var statusPaused$b = "Dijedakan";
  var statusStopped$b = "Dihentikan";
  var statusBuffering$b = "Sedang Bufer";
  var statusEnd$b = "Akhir dari trek";
  var selectedTrack$b = "Trek Pilihan";
  var alertDescribedVersion$b = "Menggunakan versi dengan deskripsi audio pada video ini";
  var alertNonDescribedVersion$b = "Menggunakan versi tanpa deskripsi pada video ini";
  var prefMenuCaptions$b = "Takarir";
  var prefVoicedCaptions$b = "Spoken Captions";
  var prefMenuDescriptions$b = "Deskripsi";
  var prefMenuKeyboard$b = "Kibor";
  var prefMenuTranscript$b = "Transkripsi";
  var prefTitleCaptions$b = "Preferensi Takarir";
  var prefTitleDescriptions$b = "Preferensi Deskripsi Audio";
  var prefTitleKeyboard$b = "Preferensi Kibor";
  var prefTitleTranscript$b = "Preferensi Transkripsi";
  var prefIntroDescription1$b = "Pemutar media ini mendukung deskripsi audio dengan dua cara: ";
  var prefDescription1$b = "Video ini memiliki versi alternatif berdeskripsi, deskripsi berbasis teks.";
  var prefDescription2$b = "Video ini memiliki versi alternatif video dengan deskripsi.";
  var prefDescription3$b = "Video ini memiliki deskripsi berbasis teks, dibacakan oleh pembaca layar.";
  var prefIntroDescriptionNone$b = "Video ini tidak memiliki deskripsi audio dalam format mana pun.";
  var prefIntroDescription3$b = "Gunakan isian berikut untuk memasang preferensi terkait deskripsi audio berbasis teks.";
  var prefIntroDescription4$b = "Setelah Anda menyimpan pengaturan, deskripsi audio dapat dinyala/matikan melalui tombol Deskripsi.";
  var prefIntroKeyboard1$b = "Pemutar media dalam halaman ini dapat dioperasikan dari bagian mana pun pada halaman dengan menggunakan pintasan kibor (lihat daftar di bawah).";
  var prefIntroKeyboard2$b = "Fungsi tombol kombinasi (Shift, Alt, Control) dapat dipasangkan di bawah ini.";
  var prefIntroKeyboard3$b = "CATATAN: Beberapa kombinasi tombol mungkin akan bersinggungan dengan yang digunakan oleh browser dan/atau aplikasi perangkat lunak lainnya. Cobalah berbagai kombinasi untuk menemukan kombinasi yang bekerja untuk Anda.";
  var prefHeadingKeyboard1$b = "Tombol kombinasi yang digunakan untuk pintasan";
  var prefHeadingKeyboard2$b = "Pintasan kibor saat ini";
  var prefHeadingDescription$b = "Deskripsi audio";
  var prefHeadingTextDescription$b = "Deskripsi audio berbasis teks";
  ...
  var escapeKeyFunction$b = "Tutup dialog atau menu sembulan yang tampil";
  var prefDescPause$b = "Otomatis jedakan video ketika deskripsi dimulai";
  var prefDescVisible$b = "Tampilkan deskripsi";
  var prefDescVoice$b = "Suara";
  var prefDescRate$b = "Spoken Description Rate";
  var prefCaptionRate$b = "Spoken Caption Rate";
  var prefDescPitch$b = "Titinada";
  var prefDescPitch1$b = "Sangat rendah";
  var prefDescPitch2$b = "Rendah";
  var prefDescPitch3$b = "Bawaan";
  var prefDescPitch4$b = "Tinggi";
  var prefDescPitch5$b = "Sangat tinggi";
  var sampleDescriptionText$b = "Sesuaikan pengaturan untuk mendengarkan teks ini.";
  var prefHighlight$b = "Soroti transkripsi yang sesuai saat media diputar";
  var prefTabbable$b = "Transkripsi dengan kontrol kibor";
  var prefCaptionsFont$b = "Jenis Huruf";
  var prefCaptionsColor$b = "Warna Teks";
  var prefCaptionsBGColor$b = "Warna Latar";
  var prefCaptionsSize$b = "Ukuran Huruf";
  var prefCaptionsOpacity$b = "Opasitas";
  var prefCaptionsStyle$b = "Gaya";
  var serif$b = "serif";
  var sans$b = "sans-serif";
  var cursive$b = "kursif";
  var fantasy$b = "fantasi";
  var monospace$b = "monospasi";
  var white$b = "putih";
  var yellow$b = "kuning";
  var green$b = "hijau";
  var cyan$a = "sian";
  ...
  var red$b = "merah";
  var black$b = "hitam";
  var transparent$b = "transparan";
  var solid$b = "solid";
  var captionsStylePopOn$b = "Muncul";
  var captionsStyleRollUp$b = "Bergulir naik";
  var prefCaptionsPosition$b = "Posisi";
  var captionsPositionOverlay$b = "Di atas video";
  var captionsPositionBelow$b = "Di bawah video";
  var sampleCaptionText$b = "Contoh teks takarir";
  var prefSuccess$b = "Perubahan telah disimpan.";
  var prefNoChange$b = "Anda tidak melakukan perubahan apa pun.";
  var save$b = "Simpan";
  var cancel$b = "Batal";
  var dismissButton$b = "Dismiss";
  var windowButtonLabel$b = "Opsi jendela";
  var windowMove$b = "Geser";
  ...
  var windowMoveAlert$b = "Seret atau gunakan tombol panah untuk menggeser jendela; Tekan Enter untuk berhenti";
  var windowResize$b = "Ubah ukuran";
  var windowResizeHeading$b = "Ubah Ukuran Jendela";
  var closeButtonLabel$b = "Tutup";
  var width$b = "Lebar";
  var height$b = "Tinggi";
  var resultsSummary1$b = "Anda mencari:";
  var resultsSummary2$b = "Ditemukan %1 hal yang sesuai.";
  var resultsSummary3$b = "Klik waktu yang terkait dengan suatu hal untuk memutar video dari titik tersebut.";
  var noResultsFound$b = "Tidak ada hasil yang ditemukan.";
  var searchButtonLabel$b = "Diputar pada %1";
  var hour$b = "jam";
  var minute$b = "menit";
  var second$b = "detik";
  var hours$b = "jam";
  ...
  var playerHeading$a = "Lettore multimediale";
  var audioPlayer$a = "Audio player";
  var videoPlayer$a = "Video player";
  var faster$a = "Più veloce";
  var slower$a = "Più lento";
  var play$a = "Riproduci";
  var pause$a = "Pausa";
  var restart$a = "Torna all'inizio";
  var prevTrack$a = "Traccia precedente";
  var nextTrack$a = "Traccia successiva";
  var rewind$a = "Indietro";
  var forward$a = "Avanti";
  var captions$a = "Sottotitoli";
  var showCaptions$a = "Mostra sottotitoli";
  var hideCaptions$a = "Nascondi sottotitoli";
  var captionsOff$a = "Disattiva sottotitoli";
  var showTranscript$a = "Mostra trascrizione";
  var hideTranscript$a = "Nascondi trascrizione";
  var turnOnDescriptions$a = "Attiva audiodescrizioni";
  var turnOffDescriptions$a = "Disattiva audiodescrizioni";
  var chapters$a = "Capitoli";
  var language$a = "Lingua";
  var sign$a = "Lingua dei segni";
  var showSign$a = "Mostra lingua dei segni";
  var hideSign$a = "Nascondi lingua dei segni";
  var seekbarLabel$a = "Tempo trascorso";
  var mute$a = "Muto";
  var unmute$a = "Riattiva l'audio";
  var volume$a = "Volume";
  var volumeUpDown$a = "Incremento e diminuzione volume";
  var preferences$a = "Preferenze";
  var enterFullScreen$a = "Attiva schermo intero";
  var exitFullScreen$a = "Disattiva schermo intero";
  var speed$a = "Velocità";
  var spacebar$a = "barra spaziatrice";
  var transcriptTitle$a = "Trascrizione";
  var lyricsTitle$a = "Testi";
  var autoScroll$a = "Auto scorrimento";
  var statusPlaying$a = "In riproduzione";
  var statusPaused$a = "In pausa";
  var statusStopped$a = "Fermato";
  var statusBuffering$a = "Buffering";
  var statusEnd$a = "Fine traccia";
  var selectedTrack$a = "Traccia selezionata";
  var alertDescribedVersion$a = "Sto usando la versione audiodescritta di questo video";
  var alertNonDescribedVersion$a = "Sto usando la versione non audiodescritta di questo video";
  var prefMenuCaptions$a = "Sottotitoli";
  var prefVoicedCaptions$a = "Spoken Captions";
  var prefMenuDescriptions$a = "Audioescrizioni";
  var prefMenuKeyboard$a = "Tastiera";
  var prefMenuTranscript$a = "Trascrizione";
  var prefTitleCaptions$a = "Preferenze sottotitoli";
  var prefTitleDescriptions$a = "Preferenze audiodescrizione";
  var prefTitleKeyboard$a = "Preferenze tastiera";
  var prefTitleTranscript$a = "Preferenze trascrizione";
  var prefIntroDescription1$a = "Questo lettore multimediale supporta le audiodescrizioni in due modi:";
  var prefDescription1$a = "il video corrente ha Una versione di descrizione alternativa, Descrizione testuale.";
  var prefDescription2$a = "il video corrente ha Versione di descrizione alternativa per il video.";
  var prefDescription3$a = "il video corrente ha Descrizione testuale, letta dal lettore di schermo.";
  var prefIntroDescriptionNone$a = "Il video corrente non ha audiodescrizioni.";
  var prefIntroDescription3$a = "Usa il seguente modulo per impostare le tue preferenze relative all'audiodescrizione testuale.";
  var prefIntroDescription4$a = "Dopo aver salvato le tue impostazioni, le audiodescrizioni possono essere attivate o disattivate usando i pulsanti descrizione.";
  var prefIntroKeyboard1$a = "Il lettore multimediale può essere usato dovunque in questa pagina, attraverso la tastiera. Vedi sotto per un elenco di tasti di scelta rapida.";
  var prefIntroKeyboard2$a = "I tasti di scelta rapida, (Shift, Alt, e Control) possono essere assegnati di seguito.";
  var prefIntroKeyboard3$a = "NOTA: Alcune combinazioni di tasti possono essere in conflitto con i tasti utilizzati dal tuo browser od altro software. Prova varie combinazioni di tasti per trovarne una che funzioni correttamente.";
  var prefHeadingKeyboard1$a = "Tasti di scelta rapida";
  var prefHeadingKeyboard2$a = "Tasti di scelta rapida correnti";
  var prefHeadingDescription$a = "Audiodescrizione";
  var prefHeadingTextDescription$a = "audiodescrizione testuale";
  var prefAltKey$a = "Alt";
  var prefCtrlKey$a = "Control";
  var prefShiftKey$a = "Shift";
  var prefNoKeyShortcuts$a = "Disable keyboard shortcuts";
  var escapeKey$a = "Escape";
  var escapeKeyFunction$a = "Chiude il menu o la finestra corrente";
  var prefDescPause$a = "Mette automaticamente il video in pausa, quando inizia la descrizione";
  var prefDescVisible$a = "Rende la descrizione visibile";
  var prefDescVoice$a = "Voice";
  var prefDescRate$a = "Spoken Description Rate";
  var prefCaptionRate$a = "Spoken Caption Rate";
  var prefDescPitch$a = "Pitch";
  var prefDescPitch1$a = "Very low";
  var prefDescPitch2$a = "Low";
  var prefDescPitch3$a = "Default";
  var prefDescPitch4$a = "High";
  var prefDescPitch5$a = "Very high";
  var sampleDescriptionText$a = "Adjust settings to hear this sample text.";
  var prefHighlight$a = "Evidenzia la descrizione";
  var prefTabbable$a = "Abilita la descrizione";
  var prefCaptionsFont$a = "Carattere";
  var prefCaptionsColor$a = "Colore del testo";
  var prefCaptionsBGColor$a = "Colore di sfondo";
  var prefCaptionsSize$a = "Dimensione del carattere";
  var prefCaptionsOpacity$a = "Opacità";
  var prefCaptionsStyle$a = "Stile";
  var serif$a = "serif";
  var sans$a = "sans-serif";
  var cursive$a = "corsivo";
  ...
  var red$a = "rosso";
  var black$a = "nero";
  var transparent$a = "trasparente";
  var solid$a = "solido";
  var captionsStylePopOn$a = "A comparsa";
  var captionsStyleRollUp$a = "A scorrimento";
  var prefCaptionsPosition$a = "Posizione";
  var captionsPositionOverlay$a = "Copre";
  var captionsPositionBelow$a = "Sotto al video";
  var sampleCaptionText$a = "Testo di esempio sottotitoli";
  var prefSuccess$a = "I cambiamenti sono stati salvati.";
  var prefNoChange$a = "Non è stato effettuato alcun cambiamento.";
  var save$a = "Salva";
  var cancel$a = "Annulla";
  var dismissButton$a = "Dismiss";
  var windowButtonLabel$a = "Opzioni finestra";
  var windowMove$a = "Sposta";
  ...
  var windowMoveAlert$a = "Trascina o usa le frecce per spostare la finestra. Invio per fermare.";
  var windowResize$a = "Ridimensiona";
  var windowResizeHeading$a = "Ridimensiona finestra";
  var closeButtonLabel$a = "Chiudi";
  var width$a = "Larghezza";
  var height$a = "Altezza";
  var resultsSummary1$a = "You searched for:";
  var resultsSummary2$a = "Found %1 matching items.";
  var resultsSummary3$a = "Click the time associated with any item to play the video from that point.";
  var noResultsFound$a = "No results found.";
  var searchButtonLabel$a = "Play at %1";
  var hour$a = "hour";
  var minute$a = "minute";
  var second$a = "second";
  var hours$a = "hours";
  ...
  var pause$9 = "一時停止";
  var restart$9 = "再開";
  var prevTrack$9 = "前のトラック";
  var nextTrack$9 = "次のトラック";
  var rewind$9 = "巻き戻し";
  var forward$9 = "早送り";
  var captions$9 = "キャプション";
  var showCaptions$9 = "キャプションを表示する";
  var hideCaptions$9 = "キャプションを非表示にする";
  var captionsOff$9 = "キャプションを消す";
  var showTranscript$9 = "書き起こしの表示";
  var hideTranscript$9 = "書き起こしを非表示にする";
  var turnOnDescriptions$9 = "音声解説を出す";
  var turnOffDescriptions$9 = "音声解説を出さない";
  var chapters$9 = "チャプター";
  var language$9 = "言語";
  var sign$9 = "手話";
  var showSign$9 = "手話を表示";
  var hideSign$9 = "手話を非表示";
  var seekbarLabel$9 = "タイムライン";
  var mute$9 = "消音";
  var unmute$9 = "消音解除";
  var volume$9 = "音量";
  var volumeUpDown$9 = "音量の上下";
  var preferences$9 = "設定";
  var enterFullScreen$9 = "全画面表示";
  var exitFullScreen$9 = "全画面表示の終了";
  var speed$9 = "再生速度";
  var spacebar$9 = "スペースキー";
  var transcriptTitle$9 = "書き起こし";
  var lyricsTitle$9 = "歌詞";
  var autoScroll$9 = "自動スクロール";
  var statusPlaying$9 = "再生中";
  var statusPaused$9 = "一時停止中";
  var statusStopped$9 = "停止";
  var statusBuffering$9 = "バッファリング中";
  var statusEnd$9 = "トラックの終わり";
  var selectedTrack$9 = "選択されたトラック";
  var alertDescribedVersion$9 = "この動画の音声解説付きバージョンを使います";
  var alertNonDescribedVersion$9 = "この動画の解説なしバージョンを使います";
  var prefMenuCaptions$9 = "キャプション";
  var prefVoicedCaptions$9 = "Spoken Captions";
  var prefMenuDescriptions$9 = "音声解説";
  var prefMenuKeyboard$9 = "キーボード";
  var prefMenuTranscript$9 = "字幕";
  var prefTitleCaptions$9 = "キャプションの設定";
  var prefTitleDescriptions$9 = "音声解説の設定";
  var prefTitleKeyboard$9 = "キーボードの設定";
  var prefTitleTranscript$9 = "字幕の設定";
  var prefIntroDescription1$9 = "このメディアプレイヤーは次の2つの方法で音声解説をサポートします: ";
  var prefDescription1$9 = "現在の動画では次の方法が選択可能です: 解説付きの代替バージョン テキストによる解説";
  var prefDescription2$9 = "現在の動画では次の方法が選択可能です: 解説付きの代替バージョンのビデオ";
  var prefDescription3$9 = "現在の動画では次の方法が選択可能です: テキストによる解説(スクリーンリーダーによって読み上げられる)";
  var prefIntroDescriptionNone$9 = "現在の動画にはどちらの形式の音声解説も含まれていません。";
  var prefIntroDescription3$9 = "次のフォームを使って、音声解説に関連する設定を保存できます。";
  var prefIntroDescription4$9 = "設定が保存されたら、音声解説ボタンによって音声解説の表示・非表示を切り替えることができます。";
  var prefIntroKeyboard1$9 = "このページのメディアプレイヤーは、キーボード・ショートカットを使ってこのページのどこからでも操作できます(下の一覧を参照してください)。";
  var prefIntroKeyboard2$9 = "修飾キー(Shift、Alt と Control) は以下で割り当てることができます。";
  var prefIntroKeyboard3$9 = "注意: いくつかのキーの組み合わせは、ブラウザやアプリケーション・ソフトで使われているものと衝突する可能性があります。ご利用の環境で正しく動作する、様々なキーの組み合わせを試してください。";
  var prefHeadingKeyboard1$9 = "ショートカットの修飾キー";
  var prefHeadingKeyboard2$9 = "現在のキーボード・ショートカット";
  var prefHeadingDescription$9 = "音声解説";
  var prefHeadingTextDescription$9 = "テキストによる音声解説";
  var prefAltKey$9 = "Alt";
  var prefCtrlKey$9 = "Control";
  var prefShiftKey$9 = "Shift";
  var prefNoKeyShortcuts$9 = "Disable keyboard shortcuts";
  var escapeKey$9 = "エスケープ";
  var escapeKeyFunction$9 = "現在のダイアログやポップアップメニューを閉じる";
  var prefDescPause$9 = "解説が表示されたら動画を自動的に停止する";
  var prefDescVisible$9 = "解説が見えるようにする";
  var prefDescVoice$9 = "音";
  var prefDescRate$9 = "Spoken Description Rate";
  var prefCaptionRate$9 = "Spoken Caption Rate";
  var prefDescPitch$9 = "ピッチ";
  var prefDescPitch1$9 = "非常に低い";
  var prefDescPitch2$9 = "低い";
  var prefDescPitch3$9 = "標準";
  var prefDescPitch4$9 = "高い";
  var prefDescPitch5$9 = "非常に高い";
  var sampleDescriptionText$9 = "このサンプル・テキストを聞くために設定を合わせます。";
  var prefHighlight$9 = "メディアの再生に合わせて字幕をハイライトする";
  var prefTabbable$9 = "キーボード操作可能な字幕";
  var prefCaptionsFont$9 = "フォント";
  var prefCaptionsColor$9 = "文字色";
  var prefCaptionsBGColor$9 = "背景色";
  var prefCaptionsSize$9 = "フォントサイズ";
  var prefCaptionsOpacity$9 = "不透明度";
  var prefCaptionsStyle$9 = "書式";
  var serif$9 = "セリフ";
  var sans$9 = "サンセリフ";
  ...
  var red$9 = "赤";
  var black$9 = "黒";
  var transparent$9 = "透明";
  var solid$9 = "不透明";
  var captionsStylePopOn$9 = "ポップ・オン";
  var captionsStyleRollUp$9 = "ロール・アップ";
  var prefCaptionsPosition$9 = "位置";
  var captionsPositionOverlay$9 = "オーバーレイ表示";
  var captionsPositionBelow$9 = "動画の下部";
  var sampleCaptionText$9 = "キャプション表示の例";
  var prefSuccess$9 = "変更が保存されました。";
  var prefNoChange$9 = "設定が変更されていません。";
  var save$9 = "保存";
  var cancel$9 = "キャンセル";
  var dismissButton$9 = "Dismiss";
  var windowButtonLabel$9 = "ウィンドウの設定";
  var windowMove$9 = "移動";
  ...
  var windowMoveAlert$9 = "マウスをドラッグするか矢印キーでウィンドウを移動できます; Enterで終了";
  var windowResize$9 = "サイズを変える";
  var windowResizeHeading$9 = "ウィンドウのサイズ変更";
  var closeButtonLabel$9 = "閉じる";
  var width$9 = "幅";
  var height$9 = "高さ";
  var resultsSummary1$9 = "次を検索:";
  var resultsSummary2$9 = "見つかりました %1 個の検索結果";
  var resultsSummary3$9 = "アイテムに関連付けられている時間をクリックして、その時点からビデオを再生します。";
  var noResultsFound$9 = "見つかりませんでした。";
  var searchButtonLabel$9 = "次から再生 %1";
  var hour$9 = "時間";
  var minute$9 = "分";
  var second$9 = "秒";
  var hours$9 = "時間";
  ...
  var playerHeading$8 = "Pemain media";
  var audioPlayer$8 = "Audio player";
  var videoPlayer$8 = "Video player";
  var faster$8 = "Lebih laju";
  var slower$8 = "Lebih perlahan";
  var play$8 = "Main";
  var pause$8 = "Jeda";
  var restart$8 = "Mula semula";
  var prevTrack$8 = "Trek sebelumnya";
  var nextTrack$8 = "Trek seterusnya";
  var rewind$8 = "Undur";
  var forward$8 = "Majukan";
  var captions$8 = "Sarikata";
  var showCaptions$8 = "Tunjuk sarikata";
  var hideCaptions$8 = "Sembunyi sarikata";
  var captionsOff$8 = "Sarikata dimatikan";
  var showTranscript$8 = "Tunjuk transkrip";
  var hideTranscript$8 = "Sembunyi transkrip";
  var turnOnDescriptions$8 = "Hidupkan deskripsi";
  var turnOffDescriptions$8 = "Matikan deskripsi";
  var chapters$8 = "Bab";
  var language$8 = "Bahasa";
  var sign$8 = "Bahasa isyarat";
  var showSign$8 = "Tunjuk bahasa isyarat";
  var hideSign$8 = "Sembunyi bahasa isyarat";
  var seekbarLabel$8 = "garis masa";
  var mute$8 = "Bisukan";
  var unmute$8 = "Nyahbisukan";
  var volume$8 = "Volum";
  var volumeUpDown$8 = "Naik turun volum";
  var preferences$8 = "Keutamaan";
  var enterFullScreen$8 = "Masuk skrin penuh";
  var exitFullScreen$8 = "Keluar skrin penuh";
  var speed$8 = "Kelajuan";
  var spacebar$8 = "bar ruang";
  var transcriptTitle$8 = "Transkrip";
  var lyricsTitle$8 = "Lirik";
  var autoScroll$8 = "Skrol automatik";
  var statusPlaying$8 = "Sedang dimainkan";
  var statusPaused$8 = "Dijeda";
  var statusStopped$8 = "Dihentikan";
  var statusBuffering$8 = "Memuatkan";
  var statusEnd$8 = "Akhir trek";
  var selectedTrack$8 = "Trek dipilih";
  var alertDescribedVersion$8 = "Menggunakan versi video dengan deskripsi audio";
  var alertNonDescribedVersion$8 = "Menggunakan versi video tanpa deskripsi audio";
  var prefMenuCaptions$8 = "Sarikata";
  var prefVoicedCaptions$8 = "Spoken Captions";
  var prefMenuDescriptions$8 = "Deskripsi";
  var prefMenuKeyboard$8 = "Papan kekunci";
  var prefMenuTranscript$8 = "Transkrip";
  var prefTitleCaptions$8 = "Keutamaan Sarikata";
  var prefTitleDescriptions$8 = "Keutamaan Deskripsi Audio";
  var prefTitleKeyboard$8 = "Keutamaan Papan Kekunci";
  var prefTitleTranscript$8 = "Keutamaan Transkrip";
  var prefIntroDescription1$8 = "Pemain media ini menyokong deskripsi audio dalam dua cara: ";
  var prefDescription1$8 = "Video semasa mempunyai versi berdeskripsi alternatif, deskripsi berasaskan teks.";
  var prefDescription2$8 = "Video semasa mempunyai versi video berdeskripsi alternatif.";
  var prefDescription3$8 = "Video semasa mempunyai deskripsi berasaskan teks, diumumkan oleh pembaca skrin.";
  var prefIntroDescriptionNone$8 = "Video semasa tiada deskripsi audio dalam mana-mana format.";
  var prefIntroDescription3$8 = "Gunakan borang berikut untuk menetapkan keutamaan berkaitan deskripsi audio berasaskan teks.";
  var prefIntroDescription4$8 = "Selepas anda menyimpan tetapan, deskripsi audio boleh dihidupkan/dimatikan menggunakan butang Deskripsi.";
  var prefIntroKeyboard1$8 = "Pemain media di laman web ini boleh dikendalikan dari mana-mana sahaja di halaman menggunakan pintasan papan kekunci (lihat di bawah untuk senarai).";
  var prefIntroKeyboard2$8 = "Kekunci pengubah (Shift, Alt, dan Control) boleh ditetapkan di bawah.";
  var prefIntroKeyboard3$8 = "NOTA: Sesetengah kombinasi kekunci mungkin bertindah dengan kekunci yang digunakan oleh pelayar dan/atau aplikasi lain. Cuba pelbagai kombinasi kekunci pengubah untuk mencari yang sesuai untuk anda.";
  var prefHeadingKeyboard1$8 = "Kekunci pengubah untuk pintasan";
  var prefHeadingKeyboard2$8 = "Pintasan papan kekunci semasa";
  var prefHeadingDescription$8 = "Deskripsi audio";
  var prefHeadingTextDescription$8 = "Deskripsi audio berasaskan teks";
  ...
  var escapeKeyFunction$8 = "Tutup dialog atau menu popup semasa";
  var prefDescPause$8 = "Jeda video secara automatik apabila deskripsi bermula";
  var prefDescVisible$8 = "Paparkan deskripsi";
  var prefDescVoice$8 = "Suara";
  var prefDescRate$8 = "Spoken Description Rate";
  var prefCaptionRate$8 = "Spoken Caption Rate";
  var prefDescPitch$8 = "Nada";
  var prefDescPitch1$8 = "Sangat rendah";
  var prefDescPitch2$8 = "Rendah";
  var prefDescPitch3$8 = "Lalai";
  var prefDescPitch4$8 = "Tinggi";
  var prefDescPitch5$8 = "Sangat tinggi";
  var sampleDescriptionText$8 = "Laraskan tetapan untuk mendengar teks contoh ini.";
  var prefHighlight$8 = "Serlahkan transkrip semasa media dimainkan";
  var prefTabbable$8 = "Aktifkan transkrip dengan papan kekunci";
  var prefCaptionsFont$8 = "Fon";
  var prefCaptionsColor$8 = "Warna Teks";
  var prefCaptionsBGColor$8 = "Latar Belakang";
  var prefCaptionsSize$8 = "Saiz Fon";
  var prefCaptionsOpacity$8 = "Kelegapan";
  var prefCaptionsStyle$8 = "Gaya";
  var serif$8 = "serif";
  var sans$8 = "sans-serif";
  var cursive$8 = "cursive";
  var fantasy$8 = "fantasy";
  var monospace$8 = "monospace";
  var white$8 = "putih";
  var yellow$8 = "kuning";
  var green$8 = "hijau";
  var cyan$8 = "sian";
  ...
  var red$8 = "merah";
  var black$8 = "hitam";
  var transparent$8 = "telus";
  var solid$8 = "pepejal";
  var captionsStylePopOn$8 = "Pop-on";
  var captionsStyleRollUp$8 = "Roll-up";
  var prefCaptionsPosition$8 = "Kedudukan";
  var captionsPositionOverlay$8 = "Lapisan atas";
  var captionsPositionBelow$8 = "Di bawah video";
  var sampleCaptionText$8 = "Teks contoh sarikata";
  var prefSuccess$8 = "Perubahan anda telah disimpan.";
  var prefNoChange$8 = "Anda tidak membuat sebarang perubahan.";
  var save$8 = "Simpan";
  var cancel$8 = "Batal";
  var dismissButton$8 = "Dismiss";
  var windowButtonLabel$8 = "Pilihan tetingkap";
  var windowMove$8 = "Alih";
  ...
  var windowMoveAlert$8 = "Seret atau gunakan kekunci anak panah untuk mengalih tetingkap; Enter untuk berhenti";
  var windowResize$8 = "Ubah saiz";
  var windowResizeHeading$8 = "Ubah Saiz Tetingkap";
  var closeButtonLabel$8 = "Tutup";
  var width$8 = "Lebar";
  var height$8 = "Tinggi";
  var resultsSummary1$8 = "Anda mencari:";
  var resultsSummary2$8 = "Ditemui %1 item sepadan.";
  var resultsSummary3$8 = "Klik masa yang berkaitan dengan mana-mana item untuk mainkan video dari titik itu.";
  var noResultsFound$8 = "Tiada hasil ditemui.";
  var searchButtonLabel$8 = "Main pada %1";
  var hour$8 = "jam";
  var minute$8 = "minit";
  var second$8 = "saat";
  var hours$8 = "jam";
  ...
  var playerHeading$7 = "Mediespiller";
  var audioPlayer$7 = "Audio player";
  var videoPlayer$7 = "Video player";
  var faster$7 = "Raskere";
  var slower$7 = "Saktere";
  var play$7 = "Spill av";
  var pause$7 = "Pause";
  var restart$7 = "Start på nytt";
  var prevTrack$7 = "Forrige spor";
  var nextTrack$7 = "Neste spor";
  var rewind$7 = "Spol tilbake";
  var forward$7 = "Spol fremover";
  var captions$7 = "Undertekster";
  var showCaptions$7 = "Vis undertekster";
  var hideCaptions$7 = "Skjul undertekster";
  var captionsOff$7 = "Slå av undertekster";
  var showTranscript$7 = "Vis transkripsjon";
  var hideTranscript$7 = "Skjul transkripsjon";
  var turnOnDescriptions$7 = "Slå på synstolking";
  var turnOffDescriptions$7 = "Slå av synstolking";
  var chapters$7 = "Kapitler";
  var language$7 = "Språk";
  var sign$7 = "Tegnspråk";
  var showSign$7 = "Vis tegnspråk";
  var hideSign$7 = "Skjul tegnspråk";
  var seekbarLabel$7 = "tidslinje";
  var mute$7 = "Lyd av";
  var unmute$7 = "Lyd på";
  var volume$7 = "Volum";
  var volumeUpDown$7 = "Volumkontroll";
  var preferences$7 = "Innstillinger";
  var enterFullScreen$7 = "Vis fullskjerm";
  var exitFullScreen$7 = "Avslutt fullskjerm";
  var speed$7 = "Hastighet";
  var spacebar$7 = "ordskiller";
  var transcriptTitle$7 = "Transkripsjon";
  var lyricsTitle$7 = "Teksting, verselinjer";
  var autoScroll$7 = "Automatisk rulling";
  var statusPlaying$7 = "Spiller av";
  var statusPaused$7 = "Satt på pause";
  var statusStopped$7 = "Stoppet";
  var statusBuffering$7 = "Mottar data";
  var statusEnd$7 = "Slutt på sporet";
  var selectedTrack$7 = "Valgt spor";
  var alertDescribedVersion$7 = "Bruker videoversjon med synstolking";
  var alertNonDescribedVersion$7 = "Bruker videoversjon uten synstolking";
  var prefMenuCaptions$7 = "Undertekster";
  var prefVoicedCaptions$7 = "Spoken Captions";
  var prefMenuDescriptions$7 = "Synstolking";
  var prefMenuKeyboard$7 = "Tastatur";
  var prefMenuTranscript$7 = "Transkripsjon";
  var prefTitleCaptions$7 = "Undertekstinnstillinger";
  var prefTitleDescriptions$7 = "Synstolkingsinnstillinger";
  var prefTitleKeyboard$7 = "Tastaturinnstillinger";
  var prefTitleTranscript$7 = "Transkripsjonsinnstillinger";
  var prefIntroDescription1$7 = "Denne mediespilleren støtter lydbeskrivelse på to måter: ";
  var prefDescription1$7 = "Denne videoen har en alternativ synstolket versjon, tekstbasert synstolking.";
  var prefDescription2$7 = "Denne videoen har alternativ synstolket versjon av video.";
  var prefDescription3$7 = "Denne videoen har tekstbasert synstolket versjon opplest av skjermleser.";
  var prefIntroDescriptionNone$7 = "Denne videoen har ikke synstolking i noen av formatene.";
  var prefIntroDescription3$7 = "Bruk følgende skjema for å gjøre dine valg angående tekstbasert synstolking.";
  var prefIntroDescription4$7 = "Etter at du har lagret dine valg kan synstolking slås av og på med synstolkingsknappen.";
  var prefIntroKeyboard1$7 = "Mediespilleren på denne nettsiden kan styres fra hvor som helst på siden ved hjelp av tastatursnarveier (se liste nedenfor).";
  var prefIntroKeyboard2$7 = "Metataster (Shift, Alt, og Control) kan legges til nedenfor.";
  var prefIntroKeyboard3$7 = "NB! Noen tastekombinasjoner kan komme i konflikt med taster som brukes av nettleseren din og/eller andre programmer. Prøv forskjellige tastekombinasjoner for å finne de som fungerer for deg.";
  var prefHeadingKeyboard1$7 = "Metataster som brukes til tastekombinasjoner";
  var prefHeadingKeyboard2$7 = "Nåværende tastekombinasjoner";
  var prefHeadingDescription$7 = "Synstolking";
  var prefHeadingTextDescription$7 = "Tekstbasert synstolking";
  var prefAltKey$7 = "Alt";
  var prefCtrlKey$7 = "Control";
  var prefShiftKey$7 = "Shift";
  var prefNoKeyShortcuts$7 = "Disable keyboard shortcuts";
  var escapeKey$7 = "Escape";
  var escapeKeyFunction$7 = "Lukk dialogboks eller popup-meny";
  var prefDescPause$7 = "Sett video automatisk på pause når synstolking starter";
  var prefDescVisible$7 = "Vis synstolking";
  var prefDescVoice$7 = "Stemme";
  var prefDescRate$7 = "Spoken Description Rate";
  var prefCaptionRate$7 = "Spoken Caption Rate";
  var prefDescPitch$7 = "Toneleie";
  var prefDescPitch1$7 = "Meget lavt";
  var prefDescPitch2$7 = "Lavt";
  var prefDescPitch3$7 = "Normalt";
  var prefDescPitch4$7 = "Høyt";
  var prefDescPitch5$7 = "Meget høyt";
  var sampleDescriptionText$7 = "Juster innstillingene for å høre denne eksempelteksten.";
  var prefHighlight$7 = "Marker det som leses i transkripsjon ved avspilling";
  var prefTabbable$7 = "Tastaturaktivert transkripsjon";
  var prefCaptionsFont$7 = "Skrifttype";
  var prefCaptionsColor$7 = "Skriftfarge";
  var prefCaptionsBGColor$7 = "Bakgrunn";
  var prefCaptionsSize$7 = "Skriftstørrelse";
  var prefCaptionsOpacity$7 = "Ugjennomsiktighet";
  var prefCaptionsStyle$7 = "Stil";
  var serif$7 = "serif";
  var sans$7 = "sans-serif";
  var cursive$7 = "kursiv";
  var fantasy$7 = "fantasi";
  var monospace$7 = "ikke proporsjonal";
  var white$7 = "hvit";
  var yellow$7 = "gul";
  var green$7 = "grønn";
  var cyan$7 = "cyan";
  ...
  var red$7 = "rød";
  var black$7 = "sort";
  var transparent$7 = "gjennomsiktig";
  var solid$7 = "ugjennomsiktig";
  var captionsStylePopOn$7 = "Dukker opp";
  var captionsStyleRollUp$7 = "Ruller opp";
  var prefCaptionsPosition$7 = "Posisjon";
  var captionsPositionOverlay$7 = "Legg oppå video";
  var captionsPositionBelow$7 = "Legg under video";
  var sampleCaptionText$7 = "Eksempel på undertekst";
  var prefSuccess$7 = "Dine endringer er lagret.";
  var prefNoChange$7 = "Du gjorde ingen endringer.";
  var save$7 = "Lagre";
  var cancel$7 = "Avbryt";
  var dismissButton$7 = "Dismiss";
  var windowButtonLabel$7 = "Vindusinnstillinger";
  var windowMove$7 = "Flytt";
  var windowMoveLeft$7 = "Window moved left";
  var windowMoveRight$7 = "Window moved right";
  var windowMoveUp$7 = "Window moved up";
  var windowMoveDown$7 = "Window moved down";
  var windowMoveStopped$7 = "Window move stopped";
  var transcriptControls$7 = "Transcript Window Controls";
  var signControls$7 = "Sign Language Window Controls";
  var windowMoveAlert$7 = "Dra eller bruk piltaster for å flytte vinduet; Enter-tast for å stoppe";
  var windowResize$7 = "Endre størrelse";
  var windowResizeHeading$7 = "Endre vindusstørrelse";
  var closeButtonLabel$7 = "Lukk";
  var width$7 = "Bredde";
  var height$7 = "Høyde";
  var resultsSummary1$7 = "Du søkte etter:";
  var resultsSummary2$7 = "Funnet %1 passende treff.";
  var resultsSummary3$7 = "Klikk tidspunkt på aktuelt treff for å avspille video derfra.";
  var noResultsFound$7 = "Ingen treff funnet.";
  var searchButtonLabel$7 = "Spill av fra %1";
  var hour$7 = "time";
  var minute$7 = "minutt";
  var second$7 = "sekund";
  var hours$7 = "timer";
  var minutes$7 = "minutter";
  var seconds$7 = "sekunder";
  var vtsHeading$7 = "Video Transcript Sorter";
  var vtsInstructions1$7 = "Use the Video Transcript Sorter to modify text tracks:";
  var vtsInstructions2$7 = "Reorder chapters, descriptions, captions, and/or subtitles so they appear in the proper sequence in Able Player's auto-generated transcript.";
  var vtsInstructions3$7 = "Modify content or start/end times (all are directly editable within the table).";
  var vtsInstructions4$7 = "Add new content, such as chapters or descriptions.";
  var vtsInstructions5$7 = "After editing, click the \"Save Changes\" button to generate new content for all relevant timed text files. The new text can be copied and pasted into new WebVTT files.";
  var vtsSelectLanguage$7 = "Select a language";
  var vtsSave$7 = "Generate new .vtt content";
  var vtsReturn$7 = "Return to Editor";
  var vtsCancel$7 = "Cancelling saving. Any edits you made have been restored in the VTS table.";
  var vtsRow$7 = "Row";
  var vtsKind$7 = "Kind";
  var vtsStart$7 = "Start";
  var vtsEnd$7 = "End";
  var vtsContent$7 = "Content";
  var vtsActions$7 = "Actions";
  var vtsNewRow$7 = "A new row %1 has been inserted.";
  var vtsDeletedRow$7 = "Row %1 has been deleted.";
  var vtsMovedRow$7 = "Row %1 has been moved %2 and is now Row %3.";
  var nb = {
  ...
  var playerHeading$6 = "Mediaspeler";
  var audioPlayer$6 = "Audiospeler";
  var videoPlayer$6 = "Videospeler";
  var faster$6 = "Sneller";
  var slower$6 = "Langzamer";
  var play$6 = "Afspelen";
  var pause$6 = "Pauzeren";
  var restart$6 = "Herstarten";
  var prevTrack$6 = "Vorige track";
  var nextTrack$6 = "Volgend track";
  var rewind$6 = "Terug";
  var forward$6 = "Verder";
  var captions$6 = "Ondertiteling";
  var showCaptions$6 = "Toon ondertiteling";
  var hideCaptions$6 = "Verberg ondertiteling";
  var captionsOff$6 = "Ondertiteling uit";
  var showTranscript$6 = "Toon transcript";
  var hideTranscript$6 = "Verberg transcript";
  var turnOnDescriptions$6 = "Beschrijvingen aanzetten";
  var turnOffDescriptions$6 = "Beschrijvingen uitzetten";
  var chapters$6 = "Hoofdstukken";
  var language$6 = "Taal";
  var sign$6 = "Gebarentaal";
  var showSign$6 = "Toon gebarentaal";
  var seekbarLabel$6 = "tijdlijn";
  var hideSign$6 = "Verberg gebarentaal";
  var mute$6 = "Dempen";
  var unmute$6 = "Dempen uit";
  var volume$6 = "Volume";
  var volumeUpDown$6 = "Volume hoger lager";
  var preferences$6 = "Voorkeuren";
  var enterFullScreen$6 = "Ga naar volledig scherm";
  var exitFullScreen$6 = "Verlaat volledig scherm";
  var speed$6 = "Snelheid";
  var spacebar$6 = "spatietoets";
  var transcriptTitle$6 = "Transcript";
  var lyricsTitle$6 = "Tekst";
  var autoScroll$6 = "Auto-scroll";
  var statusPlaying$6 = "Aan het spelen";
  var statusPaused$6 = "Gepauzeerd";
  var statusStopped$6 = "Gestopt";
  var statusBuffering$6 = "Aan het bufferen";
  var statusEnd$6 = "Einde van track";
  var selectedTrack$6 = "Geselecteerde track";
  var alertDescribedVersion$6 = "Versie met audiobeschrijving wordt gebruikt";
  var alertNonDescribedVersion$6 = "Versie zonder audiobeschrijving wordt gebruikt";
  var prefMenuCaptions$6 = "Ondertiteling";
  var prefVoicedCaptions$6 = "Spoken Captions";
  var prefMenuDescriptions$6 = "Beschrijvingen";
  var prefMenuKeyboard$6 = "Toetsenbord";
  var prefMenuTranscript$6 = "Transcript";
  var prefTitleCaptions$6 = "Voorkeuren ondertiteling";
  var prefTitleDescriptions$6 = "Voorkeuren audiobeschrijving";
  var prefTitleKeyboard$6 = "Voorkeuren toetsenbord";
  var prefTitleTranscript$6 = "Voorkeuren transcript";
  var prefIntroDescription1$6 = "Deze videospeler ondersteund audiobeschrijving op twee manieren: ";
  var prefDescription1$6 = "De huidige video heeft een alternatief beschreven versie, op tekst gebaseerde beschrijving.";
  var prefDescription2$6 = "De huidige video heeft Alternatieve beschreven versie van de video.";
  var prefDescription3$6 = "De huidige video heeft op tekst gebaseerde beschrijving, uitgesproken door de schermlezer.";
  var prefIntroDescriptionNone$6 = "De huidige video heeft in beide formaten geen audiobeschrijving.";
  var prefIntroDescription3$6 = "Gebruik het volgende formulier om je voorkeuren gerelateerd aan tekst-gebaseerde audiobeschrijving in te stellen.";
  var prefIntroDescription4$6 = "Na het opslaan van je instellingen, kan audiobeschrijving aan of uit gezet worden met de Beschrijving-knop.";
  var prefIntroKeyboard1$6 = "De mediaspeler op deze pagina kan vanaf elke plek binnen de pagina bestuurd worden met de toetsenbord sneltoetsen (zie de lijst hieronder).";
  var prefIntroKeyboard2$6 = "Mutatie knoppen (Shift, Alt, and Control) kunnen hier onder toegekend worden.";
  var prefIntroKeyboard3$6 = "LET OP: Sommige toetsencombinaties kunnen conflicteren met de toetsen die gebruikt worden door de browser en/of andere software applicaties. Probeer verscheidene combinaties van de mutatie knoppen om een combinatie te vinden die voor jou werkt.";
  var prefHeadingKeyboard1$6 = "Mutatie knoppen gebruikt voor sneltoetsen";
  var prefHeadingKeyboard2$6 = "Huidige toetsenbord sneltoetsen";
  var prefHeadingDescription$6 = "Audiobeschrijving";
  var prefHeadingTextDescription$6 = "Tekst-gebaseerde audiobeschrijving";
  var prefAltKey$6 = "Alt";
  var prefCtrlKey$6 = "Control";
  var prefShiftKey$6 = "Shift";
  var prefNoKeyShortcuts$6 = "Schakel sneltoetsen uit";
  var escapeKey$6 = "Escape";
  var escapeKeyFunction$6 = "Sluit huidige dialoog of pop-up menu";
  var prefDescPause$6 = "Pauzeer de video automatisch als de beschrijving aan wordt gezet";
  var prefDescVisible$6 = "Als er een tekst-gebaseerde beschrijving is, maak deze dan zichtbaar";
  var prefDescVoice$6 = "Stem";
  var prefDescRate$6 = "Spoken Description Rate";
  var prefCaptionRate$6 = "Spoken Caption Rate";
  var prefDescPitch$6 = "Toonhoogte";
  var prefDescPitch1$6 = "Zeer laag";
  var prefDescPitch2$6 = "Laag";
  var prefDescPitch3$6 = "Normaal";
  var prefDescPitch4$6 = "Hoog";
  var prefDescPitch5$6 = "Zeer hoog";
  var sampleDescriptionText$6 = "Pas de instellingen aan om deze voorbeeldtekst te beluisteren.";
  var prefHighlight$6 = "Highlight transcript terwijl media speelt";
  var prefTabbable$6 = "Maak transcript bedienbaar met toetsenbord";
  var prefCaptionsFont$6 = "Lettertype";
  var prefCaptionsColor$6 = "Tekstkleur";
  var prefCaptionsBGColor$6 = "Achtergrondkleur";
  var prefCaptionsSize$6 = "Grootte lettertype ";
  var prefCaptionsOpacity$6 = "Ondoorzichtigheid";
  var prefCaptionsStyle$6 = "Stijl";
  var serif$6 = "serif";
  var sans$6 = "sans-serif";
  var cursive$6 = "cursief";
  var fantasy$6 = "fantasy";
  var monospace$6 = "monospace";
  var white$6 = "wit";
  var yellow$6 = "geel";
  var green$6 = "groen";
  var cyan$6 = "cyaan";
  ...
  var red$6 = "rood";
  var black$6 = "zwart";
  var transparent$6 = "transparant";
  var solid$6 = "solide";
  var captionsStylePopOn$6 = "Verschijn";
  var captionsStyleRollUp$6 = "Rol omhoog";
  var prefCaptionsPosition$6 = "Positie";
  var captionsPositionOverlay$6 = "Overlay";
  var captionsPositionBelow$6 = "Onder video";
  var sampleCaptionText$6 = "Voorbeeld ondertiteling";
  var prefSuccess$6 = "Je wijzigingen zijn opgeslagen.";
  var prefNoChange$6 = "Je hebt geen wijzigingen gemaakt.";
  var save$6 = "Opslaan";
  var cancel$6 = "Annuleren";
  var dismissButton$6 = "Verwijder";
  var windowButtonLabel$6 = "Instellingen venster";
  var windowMove$6 = "Verplaats";
  var windowMoveLeft$6 = "Venster naar links verschoven";
  var windowMoveRight$6 = "Venster naar rechts verschoven";
  var windowMoveUp$6 = "Venster omhoog geschoven.";
  var windowMoveDown$6 = "Venster omlaag geschoven.";
  var windowMoveStopped$6 = "Verplaatsing venster gestopt";
  var transcriptControls$6 = "Bediening venster transcript";
  var signControls$6 = "Bediening venster gebarentaal";
  var windowMoveAlert$6 = "Versleep of gebruik de pijltjestoetsen om te verplaatsen. Druk op Enter om te stoppen.";
  var windowResize$6 = "Verkleinen of vergroten";
  var windowResizeHeading$6 = "Verander grootte van venster";
  var closeButtonLabel$6 = "Sluit";
  var width$6 = "Breedte";
  var height$6 = "Hoogte";
  var resultsSummary1$6 = "Je zocht naar:";
  var resultsSummary2$6 = "Er zijn %1 overeenkomende items gevonden.";
  var resultsSummary3$6 = "Klik op de tijd die bij een item staat om de video vanaf dat punt af te spelen.";
  var noResultsFound$6 = "Geen resultaten gevonden.";
  var searchButtonLabel$6 = "Speel af op %1";
  var hour$6 = "uur";
  var minute$6 = "minuut";
  var second$6 = "seconde";
  var hours$6 = "hours";
  var minutes$6 = "minuten";
  var seconds$6 = "seconden";
  var vtsHeading$6 = "Video Transcript Sorter";
  var vtsInstructions1$6 = "Gebruik de Video Transcript Sorter om teksttracks te wijzigen:";
  var vtsInstructions2$6 = "Herschik hoofdstukken, beschrijvingen, bijschriften en/of ondertiteling zodat ze in de juiste volgorde verschijnen in het automatisch gegenereerde transcript van Able Player.";
  var vtsInstructions3$6 = "Wijzig de inhoud of de begin-/eindtijden (alles is direct bewerkbaar in de tabel).";
  var vtsInstructions4$6 = "Voeg nieuwe inhoud toe, zoals hoofdstukken of beschrijvingen.";
  var vtsInstructions5$6 = "Klik na het bewerken op de knop 'Wijzigingen opslaan' om nieuwe inhoud te genereren voor alle relevante getimede tekstbestanden. De nieuwe tekst kan worden gekopieerd en geplakt in nieuwe WebVTT-bestanden.";
  var vtsSelectLanguage$6 = "Selecteer een taal";
  var vtsSave$6 = "Genereer nieuwe .vtt inhoud";
  var vtsReturn$6 = "Terug naar de editor";
  var vtsCancel$6 = "Opslaan annuleren. Alle bewerkingen die je hebt gedaan, zijn hersteld in de VTS-tabel";
  var vtsRow$6 = "Rij";
  var vtsKind$6 = "Sorteer";
  var vtsStart$6 = "Start";
  var vtsEnd$6 = "Eind";
  var vtsContent$6 = "Inhoud";
  var vtsActions$6 = "Acties";
  var vtsNewRow$6 = "Er is een nieuwe rij %1 ingevoegd.";
  var vtsDeletedRow$6 = "Rij %1 is verwijderd.";
  var vtsMovedRow$6 = "Rij %1 is verplaatst naar %2 en is nu rij %3.";
  var nl = {
  ...
  var playerHeading$5 = "Odtwarzacz mediów";
  var audioPlayer$5 = "Audio player";
  var videoPlayer$5 = "Video player";
  var faster$5 = "Szybciej";
  var slower$5 = "Wolniej";
  var play$5 = "Odtwarzaj";
  var pause$5 = "Pauza";
  var restart$5 = "Restart";
  var prevTrack$5 = "Poprzednia ścieżka";
  var nextTrack$5 = "Następna ścieżka";
  var rewind$5 = "Przewiń w tył";
  var forward$5 = "Przewiń w przód";
  var captions$5 = "Napisy";
  var showCaptions$5 = "Pokaż napisy";
  var hideCaptions$5 = "Ukryj napisy";
  var captionsOff$5 = "Wyłącz napisy";
  var showTranscript$5 = "Pokaż transkrypcję";
  var hideTranscript$5 = "Ukryj transkrypcję";
  var turnOnDescriptions$5 = "Włącz opis audio";
  var turnOffDescriptions$5 = "Wyłącz opis audio";
  var chapters$5 = "Rozdziały";
  var language$5 = "Język";
  var sign$5 = "Język migowy";
  var showSign$5 = "Pokaż język migowy";
  var hideSign$5 = "Ukryj język migowy";
  var seekbarLabel$5 = "Linia czasu";
  var mute$5 = "Wycisz";
  var unmute$5 = "Pogłośnij";
  var volume$5 = "Głośność";
  var volumeUpDown$5 = "Głośniej/Ciszej";
  var preferences$5 = "Preferencje";
  var enterFullScreen$5 = "Wejdź na pełny ekran";
  var exitFullScreen$5 = "Wyjdź z pełnego ekranu";
  var speed$5 = "Tempo";
  var spacebar$5 = "spacja";
  var transcriptTitle$5 = "Transkrypcja";
  var lyricsTitle$5 = "Tekst";
  var autoScroll$5 = "Auto przewijanie";
  var statusPlaying$5 = "Odtwarzanie";
  var statusPaused$5 = "Wstrzymane";
  var statusStopped$5 = "Zatrzymane";
  var statusBuffering$5 = "Buforowanie";
  var statusEnd$5 = "Koniec ścieżki";
  var selectedTrack$5 = "Wybrana ścieżka";
  var alertDescribedVersion$5 = "Użyj wersji filmu z opisem audio";
  var alertNonDescribedVersion$5 = "Użyj wersji filmu bez opisu audio";
  var prefMenuCaptions$5 = "Napisy";
  var prefVoicedCaptions$5 = "Spoken Captions";
  var prefMenuDescriptions$5 = "Audiodeskrypcja";
  var prefMenuKeyboard$5 = "Klawiatura";
  var prefMenuTranscript$5 = "Transkrypcja";
  var prefTitleCaptions$5 = "Preferencje napisów";
  var prefTitleDescriptions$5 = "Preferencje audiodeskrypcji";
  var prefTitleKeyboard$5 = "Preferencje klawiatury";
  var prefTitleTranscript$5 = "Preferencje transkrypcji";
  var prefIntroDescription1$5 = "Ten odtwarzacz mediów obsługuje audiodeskrypcję na dwa sposoby: ";
  var prefDescription1$5 = "Bieżący film ma alternatywna wersja opisu audio, tekst opisu audio.";
  var prefDescription2$5 = "Bieżący film ma alternatywna wersja wideo z opisem audio.";
  var prefDescription3$5 = "Bieżący film ma tekst opisu audio, ogłaszany przez czytnik ekranu.";
  var prefIntroDescriptionNone$5 = "Bieżący film nie ma audiodeskprycji.";
  var prefIntroDescription3$5 = "Użyj formularza poniższej, aby określić ustawienia odtwarzania tekstowego opisu audio";
  var prefIntroDescription4$5 = "Po zapisaniu ustawień opisu audio można włączać/wyłączać za pomocą przycisku Audiodeskrypcja.";
  var prefIntroKeyboard1$5 = "Odtwarzaczem mediów na tej stronie można sterować z dowolnego miejsca na stronie za pomocą skrótów klawiaturowych (lista poniżej).";
  var prefIntroKeyboard2$5 = "Klawisze modyfikujące (Shift, Alt i Control) można przypisać poniżej";
  var prefIntroKeyboard3$5 = "UWAGA: Niektóre kombinacje klawiszy mogą kolidować z klawiszami używanymi przez przeglądarkę i/lub inne aplikacje. Wypróbuj różne kombinacje klawiszy modyfikujących, aby znaleźć taką, która będzie dla Ciebie odpowiednia.";
  var prefHeadingKeyboard1$5 = "Klawisze modyfikujące używane do skrótów";
  var prefHeadingKeyboard2$5 = "Bieżące skróty klawiaturowe";
  var prefHeadingDescription$5 = "Audiodeskrypcja";
  var prefHeadingTextDescription$5 = "Tekst oparty na audiodeskrypcji";
  var prefAltKey$5 = "Alt";
  var prefCtrlKey$5 = "Control";
  var prefShiftKey$5 = "Shift";
  var prefNoKeyShortcuts$5 = "Disable keyboard shortcuts";
  var escapeKey$5 = "Escape";
  var escapeKeyFunction$5 = "Zamknij bieżące okno dialogowe lub menu podręczne";
  var prefDescPause$5 = "Automatycznie wstrzymuj wideo po rozpoczęciu audiodeskrypcji";
  var prefDescVisible$5 = "Wyświetlaj opis audio";
  var prefDescVoice$5 = "Głos";
  var prefDescRate$5 = "Spoken Description Rate";
  var prefCaptionRate$5 = "Spoken Caption Rate";
  var prefDescPitch$5 = "Wysokość";
  var prefDescPitch1$5 = "Bardzo niska";
  var prefDescPitch2$5 = "Niska";
  var prefDescPitch3$5 = "Domyślna";
  var prefDescPitch4$5 = "Wysoka";
  var prefDescPitch5$5 = "Bardzo wysoko";
  var sampleDescriptionText$5 = "Edytuj ustawienia, aby odczytać ten tekst na głos.";
  var prefHighlight$5 = "Podświetl transkrypcję podczas odtwarzania mediów";
  var prefTabbable$5 = "Włącz transkrypcję za pomocą klawiatury";
  var prefCaptionsFont$5 = "Czcionka";
  var prefCaptionsColor$5 = "Kolor tekstu";
  var prefCaptionsBGColor$5 = "Kolor tła";
  var prefCaptionsSize$5 = "Rozmiar czcionki";
  var prefCaptionsOpacity$5 = "Krycie";
  var prefCaptionsStyle$5 = "Styl";
  var serif$5 = "szeryfowa";
  var sans$5 = "bezszeryfowa";
  var cursive$5 = "pochyła";
  var fantasy$5 = "fantazyjna";
  var monospace$5 = "stała szerokość";
  var white$5 = "biały";
  var yellow$5 = "żółty";
  var green$5 = "zielony";
  var cyan$5 = "cyjan";
  var blue$5 = "niebieski";
  var magenta$5 = "magenta";
  var red$5 = "czerwony";
  var black$5 = "czarny";
  var transparent$5 = "transparentne";
  var solid$5 = "solidne";
  var captionsStylePopOn$5 = "Rozwiń";
  var captionsStyleRollUp$5 = "Zwiń";
  var prefCaptionsPosition$5 = "Pozycja";
  var captionsPositionOverlay$5 = "Nakładka";
  var captionsPositionBelow$5 = "Poniżej wideo";
  var sampleCaptionText$5 = "Przykładowy tekst napisu";
  var prefSuccess$5 = "Zmiany zostały zapisane.";
  var prefNoChange$5 = "Nie wprowadzono żadnych zmian.";
  var save$5 = "Zapisz";
  var cancel$5 = "Anuluj";
  var dismissButton$5 = "Dismiss";
  var windowButtonLabel$5 = "Opcje okna";
  var windowMove$5 = "Przenieś";
  ...
  var windowMoveAlert$5 = "Przeciągnij lub użyj strzałek, aby przesunąć okno; Enter, aby zatrzymać";
  var windowResize$5 = "Zmień rozmiar";
  var windowResizeHeading$5 = "Zmień rozmiar okna";
  var closeButtonLabel$5 = "Zamknij";
  var width$5 = "Szerokość";
  var height$5 = "Wysokość";
  var resultsSummary1$5 = "Szukano:";
  var resultsSummary2$5 = "Znaleziono %1 pasujące elementy.";
  var resultsSummary3$5 = "Kliknij czas powiązany z dowolnym elementem, aby odtworzyć wideo od tego momentu.";
  var noResultsFound$5 = "Nie znaleziono żadnych wyników";
  var searchButtonLabel$5 = "Odtwarzaj od %1";
  var hour$5 = "godzina";
  var minute$5 = "minuta";
  var second$5 = "sekunda";
  var hours$5 = "godziny";
  var minutes$5 = "minuty";
  var seconds$5 = "sekundy";
  ...
  var playerHeading$4 = "Reprodutor de mídias";
  ...
  var nextTrack$4 = "Próxima faixa";
  var rewind$4 = "Retroceder";
  var forward$4 = "Avançar";
  var captions$4 = "Legendas";
  var showCaptions$4 = "Mostrar legendas";
  var hideCaptions$4 = "Ocultar legendas";
  var captionsOff$4 = "Legendas desligadas";
  var showTranscript$4 = "Mostrar transcrição";
  var hideTranscript$4 = "Ocultar transcrição";
  var turnOnDescriptions$4 = "Habilitar descrições";
  var turnOffDescriptions$4 = "Desabilitar descrições";
  var chapters$4 = "Capítulos";
  var language$4 = "Idioma";
  var sign$4 = "Língua de sinais";
  var showSign$4 = "Mostrar língua de sinais";
  var hideSign$4 = "Ocultar língua de sinais";
  var seekbarLabel$4 = "Linha do tempo";
  var mute$4 = "Silenciar";
  var unmute$4 = "Ativar som";
  var volume$4 = "Volume";
  var volumeUpDown$4 = "Ampliar/reduzir volume";
  var preferences$4 = "Preferências";
  var enterFullScreen$4 = "Entrar em tela cheia";
  var exitFullScreen$4 = "Sair da tela cheia";
  var speed$4 = "Velocidade";
  var spacebar$4 = "barra de espaço";
  var transcriptTitle$4 = "Transcrição";
  var lyricsTitle$4 = "Letras";
  var autoScroll$4 = "Rolagem automática";
  var statusPlaying$4 = "Reproduzindo";
  var statusPaused$4 = "Pausado";
  var statusStopped$4 = "Parado";
  var statusBuffering$4 = "Carregando";
  var statusEnd$4 = "Fim da faixa";
  var selectedTrack$4 = "Faixa Selecionada";
  var alertDescribedVersion$4 = "Usando a versão com audiodescrição deste vídeo";
  var alertNonDescribedVersion$4 = "Utilizando a versão sem audiodescrição desse vídeo";
  var prefMenuCaptions$4 = "Capítulos";
  ...
  var prefTitleCaptions$4 = "Preferências de Legenda";
  var prefTitleDescriptions$4 = "Preferências da Audiodescrição";
  var prefTitleKeyboard$4 = "Preferências do Teclado";
  var prefTitleTranscript$4 = "Preferências da Transcrição";
  var prefIntroDescription1$4 = "Esse reprodutor de mídias suportam audiodescrição de duas maneiras: ";
  var prefDescription1$4 = "O vídeo atual está uma versão alternativa de descrição, texto baseado em descrição.";
  var prefDescription2$4 = "O vídeo atual está versão alternativa de descrição do vídeo.";
  var prefDescription3$4 = "O vídeo atual está texto baseado em descrição, anunciado pelo leitor de tela.";
  var prefIntroDescriptionNone$4 = "O vídeo atual não possui audiodescrição em nenhum formato.";
  var prefIntroDescription3$4 = "Use o formulário a seguir para definir suas preferências relacionadas à texto baseado em audiodescrição.";
  var prefIntroDescription4$4 = "Depois que salvar suas configurações, a audiodescrição poderá ser ligada/desligada usando o botão Descrição.";
  var prefIntroKeyboard1$4 = "O reprodutor de mídias desse página web pode ser operado de qualquer lugar nessa página, usando os atalhos de teclado (veja a lista abaixo).";
  var prefIntroKeyboard2$4 = "Modificar teclas (Shift, Alt and Control) podem ser gravadas abaixo.";
  var prefIntroKeyboard3$4 = "OBSERVE: Algumas combinações de teclado podem entrar em conflito com atalhos usados pelo seu navegador e/ou outro software. Teste várias combinações de atalhos de teclado para encontrar uma que funcione bem para você.";
  var prefHeadingKeyboard1$4 = "Teclas modificadas dos atalhos";
  var prefHeadingKeyboard2$4 = "Atual atalho de teclado";
  var prefHeadingDescription$4 = "Audiodescrição";
  var prefHeadingTextDescription$4 = "Texto baseado em audiodescrição";
  var prefAltKey$4 = "Alt";
  var prefCtrlKey$4 = "Control";
  var prefShiftKey$4 = "Shift";
  var prefNoKeyShortcuts$4 = "Disable keyboard shortcuts";
  var escapeKey$4 = "ESC";
  var escapeKeyFunction$4 = "Fechar diálogo ou caixa pop-up atual";
  var prefDescPause$4 = "Automaticamente pausar um vídeo quando a descrição iniciar";
  var prefDescVisible$4 = "Fazer com que a descrição esteja visível";
  ...
  var sampleDescriptionText$4 = "Ajuste as configurações para ouvir este exemplo de texto.";
  var prefHighlight$4 = "Destacar a descrição conforme a mídia for reproduzida";
  var prefTabbable$4 = "Transcrição de teclado habilitada";
  var prefCaptionsFont$4 = "Fonte";
  var prefCaptionsColor$4 = "Cor do Texto";
  var prefCaptionsBGColor$4 = "Cor do plano de Fundo";
  var prefCaptionsSize$4 = "Tamanho da Fonte";
  var prefCaptionsOpacity$4 = "transparência";
  var prefCaptionsStyle$4 = "Estilo";
  var serif$4 = "serifada";
  var sans$4 = "sem serifa";
  var cursive$4 = "cursiva";
  var fantasy$4 = "fantasiosa";
  var monospace$4 = "monoespaçada";
  var white$4 = "branca";
  var yellow$4 = "amarela";
  var green$4 = "verde";
  var cyan$4 = "ciano";
  var blue$4 = "azul";
  ...
  var solid$4 = "sólida";
  var captionsStylePopOn$4 = "Aparecer";
  var captionsStyleRollUp$4 = "Rolar pra cima";
  var prefCaptionsPosition$4 = "Posição";
  var captionsPositionOverlay$4 = "Sobrepôr";
  var captionsPositionBelow$4 = "Abaixo do vídeo";
  var sampleCaptionText$4 = "Um texto de exemplo para legendas";
  var prefSuccess$4 = "Suas alterações foram salvas";
  var prefNoChange$4 = "Você não fez nenhuma alteração";
  var save$4 = "Salvar";
  ...
  var windowMoveAlert$4 = "Arraste ou use as setas do seu teclado para mover; Enter para parar";
  var windowResize$4 = "Redimensionar";
  var windowResizeHeading$4 = "Redimensionar Janela";
  var closeButtonLabel$4 = "Fechar";
  var width$4 = "Largura";
  var height$4 = "Altura";
  var resultsSummary1$4 = "Você buscou por:";
  var resultsSummary2$4 = "Encontrado %1 itens correspondentes";
  var resultsSummary3$4 = "Ative o item de tempo associado para reproduzir o vídeo a partir desse ponto.";
  var noResultsFound$4 = "Nenhum resultado encontrado";
  var searchButtonLabel$4 = "Tocar no %1";
  var hour$4 = "hora";
  var minute$4 = "minuto";
  var second$4 = "segundos";
  var hours$4 = "horas";
  ...
  var playerHeading$3 = "Leitor multimédia";
  var audioPlayer$3 = "Audio player";
  var videoPlayer$3 = "Video player";
  var faster$3 = "Mais rápido";
  var slower$3 = "Mais lento";
  var play$3 = "Reproduzir";
  var pause$3 = "Pausa";
  var restart$3 = "Reiniciar";
  var prevTrack$3 = "Faixa anterior";
  var nextTrack$3 = "Faixa seguinte";
  var rewind$3 = "Retroceder";
  var forward$3 = "Avançar";
  var captions$3 = "Legendas";
  var showCaptions$3 = "Mostrar legendas";
  var hideCaptions$3 = "Esconder legendas";
  var captionsOff$3 = "Desativar legendas";
  var showTranscript$3 = "Mostrar transcrição";
  var hideTranscript$3 = "Esconder transcrição";
  var turnOnDescriptions$3 = "Ativar descrições";
  var turnOffDescriptions$3 = "Desativar descrições";
  var chapters$3 = "Capítulos";
  var language$3 = "Idioma";
  var sign$3 = "Linguagem gestual";
  var showSign$3 = "Mostrar linguagem gestual";
  var hideSign$3 = "Esconder linguagem gestual";
  var seekbarLabel$3 = "linha temporal";
  var mute$3 = "Desativar som";
  var unmute$3 = "Ativar som";
  var volume$3 = "Volume";
  var volumeUpDown$3 = "Volume up down";
  var preferences$3 = "Preferências";
  var enterFullScreen$3 = "Ativar a vista de ecrã inteiro";
  var exitFullScreen$3 = "Sair da vista de ecrã inteiro";
  var speed$3 = "Velocidade";
  var spacebar$3 = "barra de espaço";
  var transcriptTitle$3 = "Transcrição";
  var lyricsTitle$3 = "Letra";
  var autoScroll$3 = "Scroll automático";
  var statusPlaying$3 = "A reproduzir";
  var statusPaused$3 = "Pausado";
  var statusStopped$3 = "Parado";
  var statusBuffering$3 = "A carregar";
  var statusEnd$3 = "Fim da faixa";
  var selectedTrack$3 = "Faixa selecionada";
  var alertDescribedVersion$3 = "A usar versão de áudio descrita deste vídeo";
  var alertNonDescribedVersion$3 = "A usar versão não descrita deste vídeo";
  var prefMenuCaptions$3 = "Legendas";
  var prefVoicedCaptions$3 = "Spoken Captions";
  var prefMenuDescriptions$3 = "Descrições";
  var prefMenuKeyboard$3 = "Teclado";
  var prefMenuTranscript$3 = "Transcrição";
  var prefTitleCaptions$3 = "Preferências das Legendas";
  var prefTitleDescriptions$3 = "Preferências da Descrição de Aúdio";
  var prefTitleKeyboard$3 = "Preferências de Peclado";
  var prefTitleTranscript$3 = "Preferências de Transcrição";
  var prefIntroDescription1$3 = "Este leitor multimédia suporta descrição de áudio de duas formas: ";
  var prefDescription1$3 = "O vídeo atual tem uma versão descrita alternativa, descrição à base de texto.";
  var prefDescription2$3 = "O vídeo atual tem versão descrita alernativa do vídeo.";
  var prefDescription3$3 = "O vídeo atual tem descrição à base de texto, anunciada pelo leitor de ecrãs.";
  var prefIntroDescriptionNone$3 = "O vídeo atual não tem descrição de áudio em nenhum formato.";
  var prefIntroDescription3$3 = "Utiliza o seguinte formulário para definir as tuas preferências relacionadas com descrição de áudio à base de texto.";
  var prefIntroDescription4$3 = "Depois de guardar as tuas definições, descrição de áudio pode ser alternada on/off usando o botão Descrição.";
  var prefIntroKeyboard1$3 = "O leitor multimédia nesta página pode ser operado de qualquer lugar na página utilizando os atalhos de teclado (vê em baixo a lista).";
  var prefIntroKeyboard2$3 = "Teclas modificadoras (Shift, Alt, and Control) podem ser atribuídas em baixo.";
  var prefIntroKeyboard3$3 = "NOTA: Algumas combinações de teclas podem entrar em conflito com teclas usadas pelo teu navegador e/ou outras aplicações de software. Tenta várias combinações de teclas modificadoras para encontrar uma que funciona para ti.";
  var prefHeadingKeyboard1$3 = "Teclas modificadoras usadas para atalhos";
  var prefHeadingKeyboard2$3 = "Atalhos de teclado em uso";
  var prefHeadingDescription$3 = "Descrição de áudio";
  var prefHeadingTextDescription$3 = "Descrição de áudio à base de texto";
  var prefAltKey$3 = "Alt";
  var prefCtrlKey$3 = "Control";
  var prefShiftKey$3 = "Shift";
  var prefNoKeyShortcuts$3 = "Disable keyboard shortcuts";
  var escapeKey$3 = "Escape";
  var escapeKeyFunction$3 = "Fecha o diálogo ou menu popup";
  var prefDescPause$3 = "Pausar vídeo automaticamente quando a descrição começa";
  var prefDescVisible$3 = "Torna a descrição visível";
  var prefDescVoice$3 = "Voz";
  var prefDescRate$3 = "Spoken Description Rate";
  var prefCaptionRate$3 = "Spoken Caption Rate";
  var prefDescPitch$3 = "Tom";
  var prefDescPitch1$3 = "Muito baixo";
  var prefDescPitch2$3 = "Baixo";
  var prefDescPitch3$3 = "Padrão";
  var prefDescPitch4$3 = "Alto";
  var prefDescPitch5$3 = "Muito Alto";
  var sampleDescriptionText$3 = "Ajusta as preferências para ouvir esta amostra de texto.";
  var prefHighlight$3 = "Destacar transcrição enquanto o leito está a reproduzir.";
  var prefTabbable$3 = "Transcrição ativada por teclado";
  var prefCaptionsFont$3 = "Tipo de letra";
  var prefCaptionsColor$3 = "Cor de texto";
  var prefCaptionsBGColor$3 = "Fundo";
  var prefCaptionsSize$3 = "Tamanho de letra";
  var prefCaptionsOpacity$3 = "Opacidade";
  var prefCaptionsStyle$3 = "Estilo";
  var serif$3 = "serifada";
  var sans$3 = "sem serifa";
  var cursive$3 = "cursivo";
  var fantasy$3 = "fantasia";
  var monospace$3 = "mono-espaçada";
  var white$3 = "branco";
  var yellow$3 = "amarelo";
  var green$3 = "verde";
  var cyan$3 = "ciano";
  var blue$3 = "azul";
  ...
  var prefCaptionsPosition$3 = "Posição";
  var captionsPositionOverlay$3 = "Sobreposição";
  var captionsPositionBelow$3 = "Por baixo do vídeo";
  var sampleCaptionText$3 = "Amostra texto de legenda";
  var prefSuccess$3 = "As tuas alterações foram guardadas.";
  var prefNoChange$3 = "Não fizeste nenhuma alteração.";
  var save$3 = "Guardar";
  var cancel$3 = "Cancelar";
  var dismissButton$3 = "Dismiss";
  var windowButtonLabel$3 = "Opções de janela";
  var windowMove$3 = "Mover";
  var windowMoveLeft$3 = "Window moved left";
  var windowMoveRight$3 = "Window moved right";
  var windowMoveUp$3 = "Window moved up";
  var windowMoveDown$3 = "Window moved down";
  var windowMoveStopped$3 = "Window move stopped";
  var transcriptControls$3 = "Transcript Window Controls";
  var signControls$3 = "Sign Language Window Controls";
  var windowMoveAlert$3 = "Arrasta ou usa as teclas de seta para mover a janela; Enter para parar.";
  var windowResize$3 = "Redimencionar";
  var windowResizeHeading$3 = "Redimencionar janela";
  var closeButtonLabel$3 = "Fechar";
  var width$3 = "Largura";
  var height$3 = "Altura";
  var resultsSummary1$3 = "Pesquisaste por:";
  var resultsSummary2$3 = "Encontrado %1 itens correspondentes.";
  var resultsSummary3$3 = "Clica no tempo associado a qualquer item para reproduzir o vídeo a partir daquele ponto.";
  var noResultsFound$3 = "Nenhum resultado encontrado.";
  var searchButtonLabel$3 = "Reproduzir a partir de %1";
  var hour$3 = "hora";
  var minute$3 = "minuto";
  var second$3 = "segundo";
  var hours$3 = "horas";
  var minutes$3 = "minutos";
  var seconds$3 = "segundos";
  var vtsHeading$3 = "Video Transcript Sorter";
  var vtsInstructions1$3 = "Use the Video Transcript Sorter to modify text tracks:";
  var vtsInstructions2$3 = "Reorder chapters, descriptions, captions, and/or subtitles so they appear in the proper sequence in Able Player's auto-generated transcript.";
  var vtsInstructions3$3 = "Modify content or start/end times (all are directly editable within the table).";
  var vtsInstructions4$3 = "Add new content, such as chapters or descriptions.";
  var vtsInstructions5$3 = "After editing, click the \"Save Changes\" button to generate new content for all relevant timed text files. The new text can be copied and pasted into new WebVTT files.";
  var vtsSelectLanguage$3 = "Select a language";
  var vtsSave$3 = "Generate new .vtt content";
  var vtsReturn$3 = "Return to Editor";
  var vtsCancel$3 = "Cancelling saving. Any edits you made have been restored in the VTS table.";
  var vtsRow$3 = "Row";
  var vtsKind$3 = "Kind";
  var vtsStart$3 = "Start";
  var vtsEnd$3 = "End";
  var vtsContent$3 = "Content";
  var vtsActions$3 = "Actions";
  var vtsNewRow$3 = "A new row %1 has been inserted.";
  var vtsDeletedRow$3 = "Row %1 has been deleted.";
  var vtsMovedRow$3 = "Row %1 has been moved %2 and is now Row %3.";
  var pt = {
...
//# sourceMappingURL=ableplayer.dist.js.map
